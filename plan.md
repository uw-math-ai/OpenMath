...
- [~] `def:530B` **Order relative to starting method (530B)** (§530) — `OpenMath/Chapter5/Section530.lean::HasOrderRelativeTo` (cycle 149: sorry-first scaffold lands clean. Two operators `applyStartingThenStep` (textbook `SM`) and `applyExactThenStarting` (textbook `ES`) introduced as `noncomputable def`s with sorry'd bodies — faithful evaluation requires multivariate fixed-point arguments out of scope for a structural cycle. Predicate `HasOrderRelativeTo M S _hS p` encodes Butcher's "agree to within `O(h^{p+1})`" verbatim via `Asymptotics.IsBigO (nhds (0 : ℝ))` of the difference vector against `h^{p+1}`. Non-vacuity witness `explicitEulerGLM_hasOrderZero_trivialStarting` (order 0 for explicit Euler `(s,r)=(1,1)` × trivialStartingMethod) sorry'd pending operator-body closure. Sorry count 0 → 3, all loci documented in cycle results. Three new imports (`Asymptotics.Defs`, `Calculus.Deriv.Basic`, `Section510`).)
