...
- [~] `thm:550A` **Doubly companion matrices** (§550) — `OpenMath/Chapter5/Section550.lean` (cycle 138: scaffold + `doublyCompanionMatrix` def + `alphaPoly`/`betaPoly` + axiom-clean `doublyCompanionMatrix_det_factorization_n_one` (genuine n=1 witness via `Matrix.det_fin_one`); general-n sorry-first, deferred per issue `thm_550A_general_n.md`)
