...
vector quantitative convergence chain, and
...
quantitative convergence chains. AM2–AM5 now each have closed scalar and
vector quantitative convergence chains. Cycle 441 adds
`OpenMath/LMMAM5VectorConvergence.lean`, using the now-public
`iteratedDeriv_seven_bounded_on_Icc_vec`,
`y_seventh_order_taylor_remainder_vec`, and
`derivY_sixth_order_taylor_remainder_vec` from
`LMMAB6VectorConvergence`, mirroring the AM5 scalar template at
coefficient `23325037/725760` (slackened to `33`) and growth `5L`.
The next §1.2 convergence frontier is the **AM6 scalar/vector** chain,
which will need the corresponding AB6 helper promotion and
`LMM.IsAM6Trajectory*` definitions.
