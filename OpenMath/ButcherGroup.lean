...
/-- §384 lift of the cycle 548 mixed leaf / singleton-leaf root closed
form to the quotient layer. The product `bSeriesHom` on
`BTree.node (List.replicate a BTree.leaf ++ List.replicate b (BTree.node
[BTree.leaf]))` decomposes into a pair of root-cut powersets and an
internal-cut powerset of bSeries-only summands. -/
