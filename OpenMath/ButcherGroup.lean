...
import OpenMath.ButcherGroup.Section384SlicesTripleLeaf
...
/-- §384 lift of the all-triple-leaf-children closed form to the quotient
layer. The product `bSeriesHom` on `BTree.node (List.replicate n
(BTree.node [BTree.leaf, BTree.leaf, BTree.leaf]))` decomposes into a root-cut
powerset and an inner per-kept `Fin 4` choice over the three internal leaves
of each kept triple-leaf root child. -/
        ...
        + ∑ S ∈ (Finset.univ : Finset (Fin n)).powerset,
            (q.bSeriesHom
              ...
              (∑ χ : Fin (n - S.card) → Fin 4,
                tripleLeafChoiceProductCoef (q.bSeriesHom BTree.leaf) χ *
...
`(BTree.node (List.replicate n (BTree.node [leaf, leaf, leaf]))).order
= 1 + 4 * n`. -/
...
private theorem foldr_order_append (l₁ l₂ : List BTree) :
    List.foldr (fun t n => t.order + n) 0 (l₁ ++ l₂)
      = List.foldr (fun t n => t.order + n) 0 l₁
        + List.foldr (fun t n => t.order + n) 0 l₂ := by
  induction l₁ with
  | nil => simp
  | cons hd tl ih =>
    rw [List.cons_append]
    show hd.order + List.foldr (fun t n => t.order + n) 0 (tl ++ l₂)
      = hd.order + List.foldr (fun t n => t.order + n) 0 tl
        + List.foldr (fun t n => t.order + n) 0 l₂
    rw [ih]
    ring

private theorem foldr_order_replicate_leaf (n : ℕ) :
    List.foldr (fun t n => t.order + n) 0 (List.replicate n BTree.leaf)
      = n := by
  induction n with
  | zero => simp
  | succ m ih =>
    rw [List.replicate_succ]
    simp [List.foldr, BTree.order_leaf]
    omega

private theorem foldr_order_replicate_singleton_leaf (n : ℕ) :
    List.foldr (fun t n => t.order + n) 0
  ...
  | succ m ih =>
    rw [List.replicate_succ]
    simp [List.foldr, BTree.order_node]
    omega

private theorem foldr_order_replicate_double_leaf (n : ℕ) :
    List.foldr (fun t n => t.order + n) 0
      (List.replicate n (BTree.node [BTree.leaf, BTree.leaf]))
      = 3 * n := by
  induction n with
  | zero => simp
  | succ m ih =>
...
private theorem foldr_order_replicate_triple_leaf (n : ℕ) :
    List.foldr (fun t n => t.order + n) 0
      (List.replicate n (BTree.node [BTree.leaf, BTree.leaf, BTree.leaf]))
      = 4 * n := by
  induction n with
  | zero => simp
  | succ m ih =>
...
/-- Order of a 4-segment node aggregating triple-leaf, double-leaf,
singleton-leaf, and leaf children. -/
private theorem
    order_node_replicate_triple_double_singleton_leaf (f0 f1 f2 f3 : ℕ) :
    (BTree.node
      (List.replicate f0
          (BTree.node [BTree.leaf, BTree.leaf, BTree.leaf]) ++
        List.replicate f1 (BTree.node [BTree.leaf, BTree.leaf]) ++
        List.replicate f2 (BTree.node [BTree.leaf]) ++
        List.replicate f3 BTree.leaf)).order
      = 1 + 4 * f0 + 3 * f1 + 2 * f2 + f3 := by
  rw [BTree.order_node]
  rw [foldr_order_append, foldr_order_append, foldr_order_append,
      foldr_order_replicate_triple_leaf,
      foldr_order_replicate_double_leaf,
      foldr_order_replicate_singleton_leaf,
      foldr_order_replicate_leaf]
  ring

private theorem tripleLeafChoiceFiber_sum {n : ℕ} (χ : Fin n → Fin 4) :
    tripleLeafChoiceFiber χ ⟨0, by decide⟩
      + tripleLeafChoiceFiber χ ⟨1, by decide⟩
      + tripleLeafChoiceFiber χ ⟨2, by decide⟩
      + tripleLeafChoiceFiber χ ⟨3, by decide⟩ = n := by
  classical
  have h :=
  ...
  simpa [Fin.sum_univ_four, tripleLeafChoiceFiber, Fintype.card_fin]
    using h.symm

private theorem order_tripleLeafChoiceTree
    {n : ℕ} (χ : Fin n → Fin 4) :
    (tripleLeafChoiceTree χ).order
      = 1 + 4 * tripleLeafChoiceFiber χ ⟨0, by decide⟩
        + 3 * tripleLeafChoiceFiber χ ⟨1, by decide⟩
        + 2 * tripleLeafChoiceFiber χ ⟨2, by decide⟩
        + tripleLeafChoiceFiber χ ⟨3, by decide⟩ := by
  change
    (BTree.node
      (List.replicate (tripleLeafChoiceFiber χ ⟨0, by decide⟩)
        ...
        List.replicate (tripleLeafChoiceFiber χ ⟨2, by decide⟩)
          (BTree.node [BTree.leaf]) ++
        List.replicate (tripleLeafChoiceFiber χ ⟨3, by decide⟩)
          BTree.leaf)).order = _
  rw [order_node_replicate_triple_double_singleton_leaf]
...
private theorem order_tripleLeafChoiceTree_le
  ...
  have hsum := tripleLeafChoiceFiber_sum χ
  omega

/-- Cycle 558 `G₁.mul`-direction slice on the all-triple-leaf parametric
family `BTree.node (List.replicate n (BTree.node [leaf, leaf, leaf]))`,
with order bound `1 + 4 * n ≤ p`. Each kept root child carries a `Fin 4`
inner cut/keep choice over its three internal leaves. -/
theorem product_congr_node_replicate_triple_leaf
  ...
  have htriple_pow :
      (q.bSeriesHom
        ...
        = (q'.bSeriesHom
          (BTree.node [BTree.leaf, BTree.leaf, BTree.leaf])) ^ S.card := by
    by_cases hzero : S.card = 0
    · simp [hzero]
    · have hnpos : 0 < n := by
        have hSpos : 0 < S.card := Nat.pos_of_ne_zero hzero
  ...
  have hchoice :
      r.bSeriesHom (tripleLeafChoiceTree χ)
        = r'.bSeriesHom (tripleLeafChoiceTree χ) := by
    apply hr
    apply order_tripleLeafChoiceTree_le (n := n - S.card) (p := p)
    omega
  rw [hleaf, hchoice]
