...
- [ ] `def:530B` **Order relative to starting method (530B)** (§530) — Sorry-first scaffold attempted cycle 149, rolled back cycle 150 per cycle-139 precedent. Closure requires multi-cycle infrastructure (Path A explicit-only operators with an `IsExplicit` predicate, ~2-3 cycles; OR Path B general implicit fixed-point via `ContractingWith` / `Function.IsFixedPt`, ~3-5 cycles); not viable as a single-cycle sorry-first deliverable since the operator bodies are indivisible multivariate fixed-point computations. See `.prover-state/issues/def_530B_scaffold_strategy.md`.
