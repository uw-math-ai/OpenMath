...
theorem bdf1Vec_one_step_lipschitz
  ...
  -- Apply the triangle inequality to the expression for the residual.
  have h_triangle : ‖yseq (n + 1) - y (t₀ + (n + 1) * h)‖ ≤ ‖yseq n - y (t₀ + n * h)‖ + h * ‖f (t₀ + (n + 1) * h) (yseq (n + 1)) - f (t₀ + (n + 1) * h) (y (t₀ + (n + 1) * h))‖ + ‖bdf1VecResidual h (t₀ + n * h) y‖ := by
    have h_triangle : yseq (n + 1) - y (t₀ + (n + 1) * h) = (yseq n - y (t₀ + n * h)) + h • (f (t₀ + (n + 1) * h) (yseq (n + 1)) - f (t₀ + (n + 1) * h) (y (t₀ + (n + 1) * h))) - bdf1VecResidual h (t₀ + n * h) y := by
      unfold bdf1VecResidual;
      simp +decide [ hyf, add_mul, add_assoc ];
      rw [ hy.recurrence ] ; abel_nf;
      simp +decide [ add_assoc, add_left_comm, add_comm ];
      rw [ ← hy.recurrence ] ; ring;
    rw [ h_triangle ];
    exact le_trans ( norm_sub_le _ _ ) ( add_le_add ( le_trans ( norm_add_le _ _ ) ( add_le_add ( le_rfl ) ( by rw [ norm_smul, Real.norm_of_nonneg hh ] ) ) ) le_rfl );
  nlinarith [ hf ( t₀ + ( n + 1 ) * h ) ( yseq ( n + 1 ) ) ( y ( t₀ + ( n + 1 ) * h ) ), norm_nonneg ( yseq n - y ( t₀ + n * h ) ), norm_nonneg ( bdf1VecResidual h ( t₀ + n * h ) y ) ]
