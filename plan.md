...
the Adams–Bashforth 6-step vector convergence chain, and
...
quantitative convergence chains. AM2–AM5 each have closed scalar
quantitative chains. AM2, AM3, and AM4 vector chains are closed:
cycle 438 (`OpenMath/LMMAM2VectorConvergence.lean`), cycle 439
(`OpenMath/LMMAM3VectorConvergence.lean`), and cycle 440
(`OpenMath/LMMAM4VectorConvergence.lean`). The cycle-440 AM4 vector
port reuses the now-public `iteratedDeriv_six_bounded_on_Icc_vec`,
`y_sixth_order_taylor_remainder_vec`, and
`derivY_fifth_order_taylor_remainder_vec` from `LMMAB5Convergence`,
mirroring the AM4 scalar template at coefficient `250389/21600`
(slackened to `12`) and growth `4L`.
The next §1.2 convergence frontier is the **AM5 vector** quantitative
chain (using to-be-promoted AB6 sixth/seventh-order vector Taylor
helpers from `LMMAB6VectorConvergence`), as the natural cycle-441 target.
