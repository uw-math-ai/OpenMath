    ...
    ∃ C δ : ℝ, 0 ≤ C ∧ 0 < δ ∧
      ∀ {h : ℝ}, 0 < h → h ≤ δ → ∀ n : ℕ,
        ...
        ‖bdf1VecResidual h (t₀ + (n : ℝ) * h) y‖
          ≤ C * h ^ 2 := by
  obtain ⟨ M, hM₀, hM ⟩ := iteratedDeriv_two_bounded_on_Icc_vec hy t₀ ( t₀ + T );
  refine' ⟨ 2 * M, 1, by positivity, by norm_num, fun { h } hh₁ hh₂ n hn => _ ⟩;
  apply bdf1Vec_pointwise_residual_bound hy hM;
