...
- [~] `def:381F` **P -equivalent** (§380) — `OpenMath/Chapter3/Section381.lean` (cycle 184) — `PEquivalent` + `PReducesTo` (refl-trans closure of single-step P-reduction) in `OpenMath.Chapter3.Section312.RKTableau`, plus `PEquivalent.refl` / `PEquivalent.symm` / `PEquivalent.of_pReducesTo` and a `paddedEuler` non-vacuity witness; axiom-clean. Status `partial` because the textbook def:381E "reduced method" (P-reduce then 0-reduce, iterated until irreducible) is still deferred — the current `PEquivalent` captures the P-only flavour; adding 0-reduction is a one-constructor extension once `def:381E`'s reduced-method construction lands.
