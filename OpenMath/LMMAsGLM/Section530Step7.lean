...
/-! ### §530 LMM-as-GLM order-≥ 2 witness — BDF 7-step

`bdf7` (`s = 7`, fourteen GLM input slots `Fin 14`, **implicit** with
`β_s = 420/1089 ≠ 0`, classical order 7) embeds as a GLM of order ≥ 2
using the unshifted natural Nordsieck Taylor template
`qN k = if k < s then 1 else 0`,
`q'N k = if k < s then j else 1`, and
`q''N k = if k < s then j² else 2 * j`. Same helper-extraction recipe
as BDF6GE2 (cycle 1158) with `Fin 12 → Fin 14` size bump from AM7GE2
(cycle 1170). At order 2 the implicit `β_s ≠ 0` does not require any
Nordsieck shift `C` — that enters at GE3. -/
...
/-- Boundary `k = 7` case (first past-`h·f` row) — closes with `simp`
alone; appending `norm_num` triggers "no goals to be solved", matching
the AM7GE2 / BDF6GE2 boundary nuance. -/
private theorem q''_obligation_seven :
...
theorem bdf7_toGLM_hasOrderGe2 :
    bdf7.toGLM.HasOrderGe2 := by
  refine ⟨BDF7GE2.qN, BDF7GE2.q'N, BDF7GE2.q''N,
    ?_, ?_, BDF7GE2.q'_obligation, BDF7GE2.q''_obligation⟩
