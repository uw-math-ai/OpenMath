...
#### Generic Lyapunov Infrastructure
  - [~] **Generic quadratic Lyapunov scaffold**: standalone abstract module for the discrete Lyapunov equation `Aᵀ P A − P = −Q`. Declarations: `QuadraticLyapunov.QFPosDef` / `QFPosSemidef` (quadratic-form versions of positive (semi)definiteness on `Matrix (Fin n) (Fin n) ℝ`), `QuadraticLyapunov.IsLyapunovPair A P Q` (P PD, Q PSD, `Aᵀ P A − P = −Q`), `QuadraticLyapunov.lyapNorm P x := √(x ⬝ᵥ P.mulVec x)`, the homogeneity lemma `lyapNorm_sq_smul`, the discrete-Lyapunov identity `lyapNorm_sq_step` (`(A x) ⬝ᵥ P (A x) = x ⬝ᵥ P x − x ⬝ᵥ Q x`), the step bound `lyapNorm_sq_step_le`, and the contractive-step bound `lyapNorm_sq_step_le_of_contractive` (`(A x) ⬝ᵥ P (A x) ≤ (1 − ε)(x ⬝ᵥ P x)` under `Q ⪰ ε · P` in the QF sense). The equivalence-of-norms lemma `lyapNorm_eqv` (`∃ 0 < c₀ ≤ c₁, ∀ x, c₀ · ∑ x_i² ≤ x ⬝ᵥ P x ≤ c₁ · ∑ x_i²`) is stated but its proof is deferred to a follow-up cycle (only sorry in the file; standard finite-dimensional fact via spectral theorem or compactness of unit sphere). Will be applied in subsequent cycles to BDF4–BDF6 with concretely-solved `P` matrices from `Aᵀ P A − P = −I`. (`OpenMath/QuadraticLyapunov.lean`, cycle 460)
...
same Lyapunov/global obstruction is deferred), and
...
chains. BDF4 now has closed scalar and vector truncation chains, but its
Lyapunov/global theorem is blocked by the spectral obstruction documented
in `.prover-state/issues/bdf4_lyapunov_gap.md`: the cubic factor
`25ζ³ − 23ζ² + 13ζ − 3` is irreducible over ℚ and the absolute companion
matrix has Perron radius `≈ 2.58 > 1`. The active frontier is a generic BDF
Lyapunov framework, using a quadratic discrete Lyapunov equation
`A^T P A − P = −Q`, to close BDF4–BDF6 uniformly without per-method
eigenvector tuning.
