...
/-- Focused cycle-276 target: a realized escaping down-arrow branch should
contradict the concrete rational-approximation geometry of `R`. -/
theorem cycle276_realizedDownArrowInfinityBranch_contradiction
...
theorem cycle276_noRealizedDownArrowInfinityBranch_of_concreteRationalApprox
    (R : ℂ → ℂ) (data : OrderArrowTerminationData)
    (hconcrete : IsConcreteRationalApproxToExp R data) :
    NoRealizedDownArrowInfinityBranch R := by
  intro branch
  exact cycle276_realizedDownArrowInfinityBranch_contradiction R data hconcrete branch
