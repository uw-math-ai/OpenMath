...
/-! ## §521 — LMM-side iff bridge

Structural identity: for an `s`-step LMM `m`, the §503 GLM stability
matrix `m.toGLM.stabilityMatrix z` is block-lower-triangular relative
to the `(past-y, past-h·f)` index split. This isolates the part of the
characteristic polynomial that captures the LMM's stability poly. -/

namespace LMM

variable {s : ℕ}

/-- §521 — Cross block of the LMM-as-GLM stability matrix vanishes.
The §503 embedding produces a stability matrix whose `(past-y row,
past-h·f column)` block is zero, except possibly on the **stage row**
(`(j : ℕ) + 1 = s`). For every non-stage past-y row `castAdd s j`
(with `(j : ℕ) + 1 ≠ s`), every past-h·f column entry vanishes. -/
theorem toGLM_stabilityMatrix_cross_block_zero
    (m : LMM s) (z : ℂ) (j k : Fin s)
    (hj : (j : ℕ) + 1 ≠ s) :
    m.toGLM.stabilityMatrix z
        (Fin.cast (Nat.two_mul s).symm (Fin.castAdd s j))
        (Fin.cast (Nat.two_mul s).symm (Fin.natAdd s k)) = 0 := by
  rw [toGLM_stabilityMatrix_apply]
  -- Step 1: the V-block entry is 0.
  have hV : m.toGLM.Vℂ
      (Fin.cast (Nat.two_mul s).symm (Fin.castAdd s j))
      (Fin.cast (Nat.two_mul s).symm (Fin.natAdd s k)) = 0 := by
    have hVR : m.toGLM.V
      ...
      have hcol : ((Fin.cast (Nat.two_mul s).symm (Fin.natAdd s k) :
          Fin (2 * s)) : ℕ) = s + (k : ℕ) := by
        simp [Fin.natAdd]
      rw [hcol]
      have hj_lt : (j : ℕ) + 1 < s :=
        lt_of_le_of_ne (Nat.succ_le_of_lt j.isLt) hj
      rw [if_neg (by omega)]
    show ((m.toGLM.V _ _ : ℝ) : ℂ) = 0
    rw [hVR]; simp
  -- Step 2: the B-block entry is 0 on non-stage past-y rows.
  have hB : m.toGLM.Bℂ
    ...
    have hBR : m.toGLM.B
      ...
      have hrow : Fin.cast (Nat.two_mul s)
        ...
        ext; simp
      rw [hrow, Fin.addCases_left, if_neg hj]
    show ((m.toGLM.B _ 0 : ℝ) : ℂ) = 0
    rw [hBR]; simp
  rw [hV, hB]; ring

/-- §521 — Past-h·f → past-h·f block of the LMM-as-GLM stability matrix
is the strict shift register on non-stage past-h·f rows. The non-stage
past-h·f rows have `B` row identically zero and `V` row giving the
single `1` at column `s + (j : ℕ) + 1`. -/
theorem toGLM_stabilityMatrix_natAdd_natAdd_shift
    (m : LMM s) (z : ℂ) (j k : Fin s)
    (hj : (j : ℕ) + 1 ≠ s) :
    m.toGLM.stabilityMatrix z
        (Fin.cast (Nat.two_mul s).symm (Fin.natAdd s j))
        (Fin.cast (Nat.two_mul s).symm (Fin.natAdd s k))
      = if (k : ℕ) = (j : ℕ) + 1 then 1 else 0 := by
  rw [toGLM_stabilityMatrix_apply]
  -- Step 1: the V-block entry is the desired shift.
  have hV : m.toGLM.Vℂ
      (Fin.cast (Nat.two_mul s).symm (Fin.natAdd s j))
      (Fin.cast (Nat.two_mul s).symm (Fin.natAdd s k)) =
        (if (k : ℕ) = (j : ℕ) + 1 then 1 else 0 : ℂ) := by
    have hVR : m.toGLM.V
        (Fin.cast (Nat.two_mul s).symm (Fin.natAdd s j))
        (Fin.cast (Nat.two_mul s).symm (Fin.natAdd s k)) =
          if (k : ℕ) = (j : ℕ) + 1 then (1 : ℝ) else 0 := by
      rw [toGLM_V_natAdd_shift_apply m j hj]
      have hcol : ((Fin.cast (Nat.two_mul s).symm (Fin.natAdd s k) :
          Fin (2 * s)) : ℕ) = s + (k : ℕ) := by
        simp [Fin.natAdd]
      rw [hcol]
      by_cases hk : (k : ℕ) = (j : ℕ) + 1
      · rw [if_pos (by omega), if_pos hk]
      · rw [if_neg (by omega), if_neg hk]
    show ((m.toGLM.V _ _ : ℝ) : ℂ) = _
    rw [hVR]
    by_cases hk : (k : ℕ) = (j : ℕ) + 1
    · rw [if_pos hk, if_pos hk]; simp
    · rw [if_neg hk, if_neg hk]; simp
  -- Step 2: the B-block entry is 0 on non-stage past-h·f rows.
  have hB : m.toGLM.Bℂ
...
/-- §521 — Characteristic polynomial of the LMM-as-GLM stability matrix
factors as `X^s · q` where `q` has the same roots as the LMM stability
polynomial `m.stabilityPoly · z`. The `X^s` factor comes from the strict
shift register on the non-stage past-h·f rows (sub-target 2); `q`
captures the active LMM recurrence on the past-y rows.

Sorry-first scaffold: the proof requires expressing the `(scalar μ −
stabilityMatrix z)` matrix as a `Matrix.fromBlocks` permutation parametric
in `s`, then applying `Matrix.det_fromBlocks_zero₁₂`. The cycle 632
`bdf2_toGLM_isAStable` proof established this for `s = 2`; the cycle's
job is the parametric lift. Next concrete step: build a parametric
reindexing equiv `Fin (2 * s) ≃ Fin (s + s)` and use it together with
`toGLM_stabilityMatrix_cross_block_zero` to assemble the upper-right
zero block, then expand the bottom-right shift block determinant via
`Matrix.det_of_upperTriangular`. -/
theorem toGLM_stabilityMatrix_charpoly_factor (m : LMM s) (z : ℂ) :
    ∃ q : Polynomial ℂ,
      (m.toGLM.stabilityMatrix z).charpoly = Polynomial.X ^ s * q ∧
      ∀ μ : ℂ, q.IsRoot μ ↔ m.stabilityPoly μ z = 0 := by
  sorry

/-- §521 — Forward direction of the LMM-iff-GLM A-stability bridge.
If the LMM `m` is A-stable, then its §503 GLM embedding `m.toGLM` is
A-stable. Proof: a charpoly root of `m.toGLM.stabilityMatrix z` is
either `μ = 0` (norm `0 ≤ 1`) or a root of the cofactor polynomial `q`
from `toGLM_stabilityMatrix_charpoly_factor`, in which case it is also
a root of `m.stabilityPoly · z`, hence has norm `≤ 1` by `m`'s
A-stability. -/
theorem toGLM_isAStable_of_isAStable {m : LMM s}
    (hm : m.IsAStable) : m.toGLM.IsAStable := by
  intro z hz μ hμ
  obtain ⟨q, hcharpoly, hroots⟩ := toGLM_stabilityMatrix_charpoly_factor m z
  rw [Polynomial.IsRoot, hcharpoly, Polynomial.eval_mul,
      Polynomial.eval_pow, Polynomial.eval_X] at hμ
  rcases mul_eq_zero.mp hμ with h0 | hq
  · -- μ ^ s = 0. If s = 0 we get 1 = 0 contradiction; otherwise μ = 0.
    rcases Nat.eq_zero_or_pos s with hs | hs
    · subst hs; simp at h0
    · have hμ0 : μ = 0 :=
        (pow_eq_zero_iff (Nat.pos_iff_ne_zero.mp hs)).mp h0
      rw [hμ0]; simp
  · -- q.eval μ = 0 → m.stabilityPoly μ z = 0 → ‖μ‖ ≤ 1.
    have hqroot : q.IsRoot μ := hq
    exact hm z hz μ ((hroots μ).mp hqroot)

/-- §521 — Reverse direction of the LMM-iff-GLM A-stability bridge.
If `m.toGLM` is A-stable, then `m` is A-stable. Proof: a root of
`m.stabilityPoly · z` is also a root of the cofactor polynomial `q`
(by `toGLM_stabilityMatrix_charpoly_factor`), hence a root of the
GLM stability matrix's characteristic polynomial, hence has norm
`≤ 1` by `m.toGLM`'s A-stability. -/
theorem isAStable_of_toGLM_isAStable {m : LMM s}
    (hm : m.toGLM.IsAStable) : m.IsAStable := by
  intro z hz μ hμ
  obtain ⟨q, hcharpoly, hroots⟩ := toGLM_stabilityMatrix_charpoly_factor m z
  have hqroot : q.IsRoot μ := (hroots μ).mpr hμ
  apply hm z hz μ
  rw [Polynomial.IsRoot, hcharpoly, Polynomial.eval_mul,
      Polynomial.eval_pow, Polynomial.eval_X]
  rw [Polynomial.IsRoot] at hqroot
  rw [hqroot]; ring

/-- §521 headline — A linear multistep method is A-stable iff its
§503 GLM embedding is A-stable. -/
theorem toGLM_isAStable_iff (m : LMM s) :
    m.toGLM.IsAStable ↔ m.IsAStable :=
  ⟨isAStable_of_toGLM_isAStable, toGLM_isAStable_of_isAStable⟩
