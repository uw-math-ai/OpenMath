...
- [ ] `def:530B` **Order relative to starting method (530B)** (§530) — Sorry-first scaffold attempted cycle 149, rolled back cycle 150 per cycle-139 precedent. Cycle 151: **Path A Step 1 complete** — `GeneralizedRungeKuttaMethod.IsExplicit` predicate (strict-lower-triangular `A`) plus axiom-clean non-vacuity witnesses landed in `OpenMath/Chapter5/Section530.lean` (`trivialGeneralizedRK_isExplicit` vacuous positive, `explicit2StageGRK_isExplicit` Heun-style non-vacuous positive, `implicit2StageGRK_not_isExplicit` negative). Cycle 152 target: Path A Step 2 — `applyStartingThenStep_explicit` / `applyExactThenStarting_explicit` operator bodies via direct stage-index recursion (~80-120 LOC). Cycle 153 target: Path A Step 3 — `HasOrderRelativeTo_explicit` predicate plus order-zero witness for explicit Euler × `trivialStartingMethod` (~50-80 LOC). See `.prover-state/issues/def_530B_scaffold_strategy.md`.
