import OpenMath.ButcherGroup

open Finset

namespace ButcherTableau

/-- Goal: prove the §384 honest convolution closed form on the
all-triple-leaf parametric family. -/
theorem ButcherProduct.bConv_node_replicate_triple_leaf_eq_scaffold
    {s t : ℕ} (t₁ : ButcherTableau s) (t₂ : ButcherTableau t) (n : ℕ) :
    ButcherProduct.bConv (t₁.bSeries) t₂
        (BTree.node
          (List.replicate n
            (BTree.node [BTree.leaf, BTree.leaf, BTree.leaf])))
      = t₁.bSeries
          (BTree.node
            (List.replicate n
              (BTree.node [BTree.leaf, BTree.leaf, BTree.leaf])))
        + ∑ S ∈ (Finset.univ : Finset (Fin n)).powerset,
            (t₁.bSeries
                (BTree.node [BTree.leaf, BTree.leaf, BTree.leaf]))
              ^ S.card *
              (∑ χ : Fin (n - S.card) → Fin 4,
                tripleLeafChoiceProductCoef (t₁.bSeries BTree.leaf) χ *
                  t₂.bSeries (tripleLeafChoiceTree χ)) := by
  sorry

end ButcherTableau
