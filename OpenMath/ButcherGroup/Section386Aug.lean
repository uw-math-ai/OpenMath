...
/-- Forest-only part of `bSeriesConvAug` at a node.

This is the `innerCutForest` sum from `bSeriesConvAug_node`, separated so
the associativity proof can name the list-level invariant directly. -/
private noncomputable def bSeriesConvAugForest (α β : AugSeries)
    (cs : List BTree) : ℝ :=
  ((BTree.innerCutForest cs α.toFun).map (fun forest =>
    forest.foldr (fun e acc => e.2 * acc) (1 : ℝ)
      * β.toFun (BTree.node (forest.filterMap (fun e => e.1))))).sum

/-- Forest-level associativity invariant for the augmented convolution.

The `cons` case is the remaining cycle-597 proof gap: after rewriting with
`bSeriesConvAug_innerForest_cons`, the tail split is closed by the list IH
and the residual is the nested keep-root branch on
`BTree.node (trunk :: forest.filterMap ...)`.  Closing it requires the
stronger peeled forest IH described in the cycle strategy. -/
private theorem bSeriesConvAug_forest_assoc
    ...
    bSeriesConvAugForest α β cs
      ...
      = bSeriesConvAugForest α ⟨1, fun σ => bSeriesConvAug β γ σ⟩ cs := by
  induction cs with
  | nil =>
      unfold bSeriesConvAugForest
      rw [bSeriesConvAug_innerForest_nil, bSeriesConvAug_innerForest_nil,
        bSeriesConvAug_innerForest_nil]
      simp only
      rw [bSeriesConvAug_node_nil, hγ]
      ring
  | cons c cs ih =>
      unfold bSeriesConvAugForest
      rw [bSeriesConvAug_innerForest_cons, bSeriesConvAug_innerForest_cons,
        bSeriesConvAug_innerForest_cons]
      -- Residual cycle-597 gap: peel the keep-root branch once more and
      -- apply the stronger forest IH to `trunk :: forest.filterMap ...`.
      sorry

/-- Generic unital associativity headline for the augmented B-series
convolution. -/
theorem bSeriesConvAug_assoc
    (α β γ : AugSeries) (hα : α.IsUnital) (hβ : β.IsUnital) (hγ : γ.IsUnital)
    (τ : BTree) :
    bSeriesConvAug ⟨1, fun σ => bSeriesConvAug α β σ⟩ γ τ
      = bSeriesConvAug α ⟨1, fun σ => bSeriesConvAug β γ σ⟩ τ := by
  cases τ with
  | leaf =>
      apply mul_assoc_at_leaf
      exact hα
      exact hβ
      exact hγ
  | node children =>
      rw [bSeriesConvAug_node, bSeriesConvAug_node]
      simp only
      rw [hγ, bSeriesConvAug_node α β children, hβ]
      have hforest :=
        bSeriesConvAug_forest_assoc α β γ hα hβ hγ children
      unfold bSeriesConvAugForest at hforest
      rw [← hforest]
      ring
