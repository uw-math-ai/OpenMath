...
/-! ## Adams--Moulton 3-step Quantitative Convergence Chain (Iserles §1.2)

Quantitative scalar convergence chain for the implicit Adams--Moulton
3-step method.  The chain follows the AM2 template: the implicit update is
parameterised by a supplied trajectory satisfying the AM3 recurrence, the
local residual is bounded by fifth-order Taylor remainders, and the global
...
with factor `1 - (9/24)·h·L`.  After division we use the same conservative
`3L` growth slack as AM2; the explicit AM3 weights alone have coefficient
`25L/24`, while the implicit slot necessarily contributes through the
division. -/

namespace LMM

/-- AM3 trajectory predicate:
...
structure IsAM3Trajectory (h : ℝ) (f : ℝ → ℝ → ℝ) (t₀ : ℝ)
    (y : ℕ → ℝ) : Prop where
  recurrence :
    ∀ n : ℕ, y (n + 3)
      = y (n + 2)
...
/-- AM3 local truncation operator reduces to the textbook residual
`y(t+3h) - y(t+2h) - h(9/24 y'(t+3h) + 19/24 y'(t+2h)
  - 5/24 y'(t+h) + 1/24 y'(t))`. -/
theorem am3_localTruncationError_eq
    (h t : ℝ) (y : ℝ → ℝ) :
    adamsMoulton3.localTruncationError h t y
      = y (t + 3 * h) - y (t + 2 * h)
  ...
  simp [adamsMoulton3, Fin.sum_univ_four, iteratedDeriv_one,
    iteratedDeriv_zero]
  ring
...
the divided form. -/
theorem am3_one_step_lipschitz
    {h L : ℝ} (hh : 0 ≤ h)
    (hsmall : (9 / 24 : ℝ) * h * L ≤ 1 / 2)
    {f : ℝ → ℝ → ℝ}
    (hf : ∀ t a b : ℝ, |f t a - f t b| ≤ L * |a - b|)
    (t₀ : ℝ) {yseq : ℕ → ℝ}
    (hy : IsAM3Trajectory h f t₀ yseq)
    (y : ℝ → ℝ)
    ...
    (1 - (9 / 24 : ℝ) * h * L)
      ...
      ≤ |yseq (n + 2) - y (t₀ + ((n : ℝ) + 2) * h)|
        ...
        + (1 / 24 : ℝ) * h * L
            * |yseq n - y (t₀ + (n : ℝ) * h)|
        + |adamsMoulton3.localTruncationError h
              (t₀ + (n : ℝ) * h) y| := by
  -- Abbreviations.
  ...
  set τ : ℝ := adamsMoulton3.localTruncationError h tn y with hτ_def
  ...
  have hτ_eq : τ = zn3 - zn2
      - h * ((9 / 24 : ℝ) * f tn3 zn3
             ...
             - (5 / 24 : ℝ) * f tn1 zn1
             + (1 / 24 : ℝ) * f tn zn) := by
    show adamsMoulton3.localTruncationError h tn y = _
    rw [am3_localTruncationError_eq, htn1_h, htn_2h, htn_3h,
      hyf tn3, hyf tn2, hyf tn1, hyf tn]
  -- Algebraic decomposition of the implicit global-error increment.
  have halg : yn3 - zn3
      = (yn2 - zn2)
        + (9 / 24 : ℝ) * h * (f tn3 yn3 - f tn3 zn3)
        + (19 / 24 : ℝ) * h * (f tn2 yn2 - f tn2 zn2)
        - (5 / 24 : ℝ) * h * (f tn1 yn1 - f tn1 zn1)
        + (1 / 24 : ℝ) * h * (f tn yn - f tn zn)
        - τ := by
    conv_lhs => rw [hstep]
    rw [hτ_eq]
    ring
  ...
  have hLip0 : |f tn yn - f tn zn| ≤ L * |yn - zn| :=
    hf tn yn zn
  have h9_nn : 0 ≤ (9 / 24 : ℝ) * h :=
    mul_nonneg (by norm_num) hh
  have h19_nn : 0 ≤ (19 / 24 : ℝ) * h :=
    mul_nonneg (by norm_num) hh
  have h5_nn : 0 ≤ (5 / 24 : ℝ) * h :=
    mul_nonneg (by norm_num) hh
  have h1_nn : 0 ≤ (1 / 24 : ℝ) * h :=
    mul_nonneg (by norm_num) hh
  have h9_abs :
      |(9 / 24 : ℝ) * h * (f tn3 yn3 - f tn3 zn3)|
      ≤ (9 / 24 : ℝ) * h * L * |yn3 - zn3| := by
    rw [abs_mul, abs_of_nonneg h9_nn]
    calc (9 / 24 : ℝ) * h * |f tn3 yn3 - f tn3 zn3|
        ≤ (9 / 24 : ℝ) * h * (L * |yn3 - zn3|) :=
          mul_le_mul_of_nonneg_left hLip3 h9_nn
      _ = (9 / 24 : ℝ) * h * L * |yn3 - zn3| := by ring
  have h19_abs :
      |(19 / 24 : ℝ) * h * (f tn2 yn2 - f tn2 zn2)|
      ≤ (19 / 24 : ℝ) * h * L * |yn2 - zn2| := by
    rw [abs_mul, abs_of_nonneg h19_nn]
    calc (19 / 24 : ℝ) * h * |f tn2 yn2 - f tn2 zn2|
        ≤ (19 / 24 : ℝ) * h * (L * |yn2 - zn2|) :=
          mul_le_mul_of_nonneg_left hLip2 h19_nn
      _ = (19 / 24 : ℝ) * h * L * |yn2 - zn2| := by ring
  have h5_abs :
      |(5 / 24 : ℝ) * h * (f tn1 yn1 - f tn1 zn1)|
      ≤ (5 / 24 : ℝ) * h * L * |yn1 - zn1| := by
    rw [abs_mul, abs_of_nonneg h5_nn]
    calc (5 / 24 : ℝ) * h * |f tn1 yn1 - f tn1 zn1|
        ≤ (5 / 24 : ℝ) * h * (L * |yn1 - zn1|) :=
          mul_le_mul_of_nonneg_left hLip1 h5_nn
      _ = (5 / 24 : ℝ) * h * L * |yn1 - zn1| := by ring
  have h1_abs :
      |(1 / 24 : ℝ) * h * (f tn yn - f tn zn)|
      ≤ (1 / 24 : ℝ) * h * L * |yn - zn| := by
    rw [abs_mul, abs_of_nonneg h1_nn]
    calc (1 / 24 : ℝ) * h * |f tn yn - f tn zn|
        ≤ (1 / 24 : ℝ) * h * (L * |yn - zn|) :=
  ...
  have htri_terms (A B C D E F : ℝ) :
      |A + B + C - D + E - F|
        ≤ |A| + |B| + |C| + |D| + |E| + |F| := by
    have h1 : |A + B + C - D + E - F| ≤ |A + B + C - D + E| + |F| :=
      abs_sub _ _
    have h2 : |A + B + C - D + E| ≤ |A + B + C - D| + |E| :=
      abs_add_le _ _
  ...
  have hmain :
    ...
    calc |yn3 - zn3|
      ...
      _ ≤ |yn2 - zn2|
          ...
          + (1 / 24 : ℝ) * h * L * |yn - zn|
          + |τ| := by
        linarith [h9_abs, h19_abs, h5_abs, h1_abs]
  linarith [hmain]

/-- Divided one-step AM3 error bound.  The explicit AM3 weights contribute
`25L/24`; after dividing by the implicit `(1 - (9/24)hL)` factor, we slacken
the max-window growth to `3L` and the residual coefficient to `2`. -/
theorem am3_one_step_error_bound
    {h L : ℝ} (hh : 0 ≤ h) (hL : 0 ≤ L)
    (hsmall : (9 / 24 : ℝ) * h * L ≤ 1 / 2)
    {f : ℝ → ℝ → ℝ}
    (hf : ∀ t a b : ℝ, |f t a - f t b| ≤ L * |a - b|)
    ...
    |yseq (n + 3) - y (t₀ + ((n : ℝ) + 3) * h)|
      ≤ (1 + h * (3 * L))
            * max (max |yseq n - y (t₀ + (n : ℝ) * h)|
                  ...
                  |yseq (n + 2)
                      - y (t₀ + ((n : ℝ) + 2) * h)|
        + 2 * |adamsMoulton3.localTruncationError h
              (t₀ + (n : ℝ) * h) y| := by
  ...
  set τabs : ℝ :=
    |adamsMoulton3.localTruncationError h (t₀ + (n : ℝ) * h) y|
    with hτabs_def
  set E : ℝ := max (max en en1) en2 with hE_def
  ...
  have hE_nn : 0 ≤ E := le_trans hen_nn (le_trans (le_max_left _ _) (le_max_left _ _))
  have hen_le_E : en ≤ E := le_trans (le_max_left _ _) (le_max_left _ _)
  have hen1_le_E : en1 ≤ E := le_trans (le_max_right _ _) (le_max_left _ _)
  have hen2_le_E : en2 ≤ E := le_max_right _ _
  ...
  have hstep :
    ...
    simpa [hen_def, hen1_def, hen2_def, hen3_def, hτabs_def] using
      (am3_one_step_lipschitz (h := h) (L := L) hh hsmall hf t₀ hy y hyf n)
  ...
  have hcancel :
      en3 ≤ (1 + h * (3 * L)) * E + 2 * τabs :=
    le_of_mul_le_mul_left hprod hA_pos
  exact hcancel

/-- Max-norm AM3 one-step recurrence on the rolling triple
`(en, en1, en2)`. -/
theorem am3_one_step_error_pair_bound
    {h L : ℝ} (hh : 0 ≤ h) (hL : 0 ≤ L)
    (hsmall : (9 / 24 : ℝ) * h * L ≤ 1 / 2)
    {f : ℝ → ℝ → ℝ}
    (hf : ∀ t a b : ℝ, |f t a - f t b| ≤ L * |a - b|)
  ...
  -- en3 bound from `am3_one_step_error_bound`.
  have hen3_bd :
      en3 ≤ (1 + h * (3 * L)) * E + 2 * τabs := by
    simpa [hen_def, hen1_def, hen2_def, hen3_def, hτabs_def, hE_def] using
      (am3_one_step_error_bound (h := h) (L := L) hh hL hsmall hf t₀ hy y hyf n)
  -- en1, en2 ≤ E ≤ (1 + h(3L)) * E ≤ (1 + h(3L)) * E + 2 * τabs.
  have hE_le_grow : E ≤ (1 + h * (3 * L)) * E := by
    have hone : (1 : ℝ) * E ≤ (1 + h * (3 * L)) * E :=
      mul_le_mul_of_nonneg_right (by linarith) hE_nn
    linarith
  have hen1_bd : en1 ≤ (1 + h * (3 * L)) * E + 2 * τabs := by
    linarith [hen1_le_E, hE_le_grow]
  have hen2_bd : en2 ≤ (1 + h * (3 * L)) * E + 2 * τabs := by
    linarith [hen2_le_E, hE_le_grow]
  exact max_le (max_le hen1_bd hen2_bd) hen3_bd

/-- Pointwise AM3 truncation residual bound.  The residual expands as
`R_y(3) - R_y(2) - (9h/24)R_y'(3) - (19h/24)R_y'(2)
  + (5h/24)R_y'(1)`, with `R_y'(0) = 0`.  The exact triangle-bound
coefficient is `131/32`, and we slacken to `5`. -/
theorem am3_pointwise_residual_bound
  ...
  -- R_y(2) and R_y(3), both expanded to fourth order around `t`.
  have hRy2 :=
    y_fifth_order_taylor_remainder hy hbnd ht ht2h h2h
  have hRy3 :=
    y_fifth_order_taylor_remainder hy hbnd ht ht3h h3h
  -- Remainders for `y'` at h, 2h, and 3h.
  have hRyp1 :=
  ...
  -- Algebraic identity for the AM3 residual.
  have hLTE_eq :
      y3 - y2 - h * ((9 / 24) * d3 + (19 / 24) * d2
                    - (5 / 24) * d1 + (1 / 24) * d0)
        = (y3 - y0 - (3 * h) * d0
              - (3 * h) ^ 2 / 2 * dd
              - (3 * h) ^ 3 / 6 * ddd
  ...
  have h9h_nn : 0 ≤ 9 * h / 24 := by linarith
  have h19h_nn : 0 ≤ 19 * h / 24 := by linarith
  have h5h_nn : 0 ≤ 5 * h / 24 := by linarith
  have habs9 : |(9 * h / 24) * C| = (9 * h / 24) * |C| := by
    rw [abs_mul, abs_of_nonneg h9h_nn]
  have habs19 : |(19 * h / 24) * D| = (19 * h / 24) * |D| := by
    rw [abs_mul, abs_of_nonneg h19h_nn]
  have habs5 : |(5 * h / 24) * E| = (5 * h / 24) * |E| := by
    rw [abs_mul, abs_of_nonneg h5h_nn]
  have htri : |A - B - (9 * h / 24) * C - (19 * h / 24) * D
                  + (5 * h / 24) * E|
      ≤ |A| + |B| + (9 * h / 24) * |C| + (19 * h / 24) * |D|
          + (5 * h / 24) * |E| := by
    have h1 : |A - B - (9 * h / 24) * C - (19 * h / 24) * D
                  + (5 * h / 24) * E|
        ≤ |A - B - (9 * h / 24) * C - (19 * h / 24) * D|
          + |(5 * h / 24) * E| :=
      abs_add_le _ _
    have h2 : |A - B - (9 * h / 24) * C - (19 * h / 24) * D|
        ≤ |A - B - (9 * h / 24) * C| + |(19 * h / 24) * D| :=
      abs_sub _ _
    have h3 : |A - B - (9 * h / 24) * C|
        ≤ |A - B| + |(9 * h / 24) * C| := abs_sub _ _
    have h4 : |A - B| ≤ |A| + |B| := abs_sub _ _
    linarith [habs9.le, habs9.ge, habs19.le, habs19.ge,
      habs5.le, habs5.ge]
  ...
  have h9C_bd : (9 * h / 24) * |C| ≤ (9 * h / 24) * (M / 24 * (3 * h) ^ 4) :=
    mul_le_mul_of_nonneg_left hC_bd h9h_nn
  have h19D_bd : (19 * h / 24) * |D| ≤ (19 * h / 24) * (M / 24 * (2 * h) ^ 4) :=
    mul_le_mul_of_nonneg_left hD_bd h19h_nn
  have h5E_bd : (5 * h / 24) * |E| ≤ (5 * h / 24) * (M / 24 * h ^ 4) :=
    mul_le_mul_of_nonneg_left hE_bd h5h_nn
  -- Sum of upper bounds is 131/32, slackened to 5.
  have hbound_alg :
      M / 120 * (3 * h) ^ 5 + M / 120 * (2 * h) ^ 5
        + (9 * h / 24) * (M / 24 * (3 * h) ^ 4)
        + (19 * h / 24) * (M / 24 * (2 * h) ^ 4)
        + (5 * h / 24) * (M / 24 * h ^ 4)
        = (131 / 32 : ℝ) * M * h ^ 5 := by ring
  have hMnn : 0 ≤ M := by
    have := hbnd t ht
  ...
  have hslack : (131 / 32 : ℝ) * M * h ^ 5 ≤ 5 * M * h ^ 5 := by
    ...
    linarith [this]
  linarith [htri, hA_bd, hB_bd, h9C_bd, h19D_bd, h5E_bd,
    hbound_alg.le, hbound_alg.ge, hslack]

/-- Uniform bound on the AM3 one-step truncation residual on a finite
horizon, given a `C^5` exact solution. -/
theorem am3_local_residual_bound
  ...
  -- with `k ≤ 3` (using `(n+3)*h ≤ T` and `h ≤ 1`).
  obtain ⟨M, hM_nn, hM⟩ :=
  ...
  rw [am3_localTruncationError_eq]
  ...
  have hreshape :
      h * (9 / 24 * deriv y (t + 3 * h)
        ...
        = h * ((9 / 24) * deriv y (t + 3 * h)
              + (19 / 24) * deriv y (t + 2 * h)
              - (5 / 24) * deriv y (t + h)
              + (1 / 24) * deriv y t) := by ring
  rw [hreshape]
  exact am3_pointwise_residual_bound hy hM ht_mem hth_mem ht2h_mem
    ht3h_mem hh.le

/-- Headline AM3 global error bound.  Given a supplied AM3 trajectory and
starting errors bounded by `ε₀`, the scalar global error is `O(ε₀ + h^4)` on
a finite horizon. -/
theorem am3_global_error_bound
  ...
  obtain ⟨C, δ, hC_nn, hδ_pos, hresidual⟩ :=
    am3_local_residual_bound hy_smooth t₀ T hT
  ...
  -- EN (n+1) ≤ (1 + h*(3L)) * EN n + 2*C*h^5.
  have hstep_general : ∀ n : ℕ, ((n : ℝ) + 3) * h ≤ T →
      EN (n + 1) ≤ (1 + h * (3 * L)) * EN n + (2 * C) * h ^ 5 := by
    intro n hnh_le
    have honestep := am3_one_step_error_pair_bound
    ...
    have h2τ : 2 * |adamsMoulton3.localTruncationError h
        (t₀ + (n : ℝ) * h) y| ≤ (2 * C) * h ^ 5 := by
      have h2nn : (0 : ℝ) ≤ 2 := by norm_num
      have := mul_le_mul_of_nonneg_left hres h2nn
      linarith [this]
    linarith [honestep, h2τ]
  -- Base cases N = 0, 1, 2 are direct from the starting bounds.
