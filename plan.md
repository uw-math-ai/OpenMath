...
- [~] `lem:441A` **Maximum order for a convergent k-step method** (§441) — cycle 173 added algebraic identities `aPoly_coeff_zero_of_preconsistent`, `aPoly_coeff_one_unconditional`, `aPoly_coeff_one_eq_neg_two_alpha_deriv_at_one_of_preconsistent`; cycle 174 added `ρPoly` definition + bridge lemmas + headline corollary `aPoly_coeff_one_eq_two_rho_deriv_at_one_of_preconsistent` (`a₁ = 2·ρ'(1)`); cycle 175 added Phase B.1.β `ρPoly_no_real_root_gt_one` (stability ⇒ no real root > 1, axiom-clean) + BDF2 sanity (`a₁ = 4/3`); cycle 176 added Phase B.2 `ρPoly_deriv_eval_one_ne_zero_of_stable_preconsistent` (`ρ'(1) ≠ 0`, axiom-clean) via private aux `idSeq_isHomogeneousSolution_of_preconsistent_ρPoly_deriv_zero` + BDF2 sanity `bdf2LMM_ρPoly_deriv_eval_one_eq` (`ρ'(1) = 2/3`); cycle 177 added Phase B.3 Step 1 `ρPoly_pos_on_Ioi_one` (stable + preconsistent + `0 < k` ⇒ `ρ > 0` on `(1, ∞)`, axiom-clean) via four private leading-coefficient helpers (`ρPoly_coeff_top_eq_one`, `ρPoly_natDegree_eq_k`, `ρPoly_leadingCoeff_eq_one`, `ρPoly_tendsto_atTop`) + IVT closure on cycle 175's no-real-root-gt-1, plus BDF2 sanity `bdf2LMM_ρPoly_pos_at_two = 5/3 > 0`; cycle 178 added Phase B.3 Step 2 `ρPoly_deriv_eval_one_pos_of_stable_preconsistent` (stable + preconsistent + `0 < k` ⇒ `ρ'(1) > 0`, axiom-clean) via `Polynomial.hasDerivAt` + `HasDerivAt.tendsto_slope` restricted to `nhdsWithin 1 (Set.Ioi 1)` + `ge_of_tendsto` + `lt_of_le_of_ne` (combining cycle 174's `ρ(1) = 0`, cycle 176's `ρ'(1) ≠ 0`, and cycle 177's `ρ > 0` on `(1, ∞)`), plus BDF2 sanity `bdf2LMM_ρPoly_deriv_eval_one_pos` (`0 < 2/3` via cycle 176 closed form); cycle 179 closed Phase B.4 with `aPoly_coeff_one_pos_of_stable_preconsistent` (stable + preconsistent + `0 < k` ⇒ `0 < a₁`, axiom-clean) via cycle 174 bridge `a₁ = 2·ρ'(1)` + cycle 178 positivity + `linarith`, plus BDF2 sanity `bdf2LMM_aPoly_coeff_one_pos` (`0 < 4/3` via cycle 175 closed form); the `a₁ > 0` half of `lem:441A` is now fully closed; Phase C (`aᵢ ≥ 0` for `i ≥ 2`, complex-root decomposition) is multi-cycle work deferred to a fresh strategy
