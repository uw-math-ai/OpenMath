...
/-! ### Cycle 592: parametric combinator API for `node (replicate n (node [leaf]))`

Three-way per-position cut on a node whose children are all `node [leaf]`:
each position is either fully cut at the child root (recorded in `S₁`),
partial-trunk cut keeping the root and pruning the inner leaf
(recorded in `S₂`), or fully kept. -/

/-- Disjoint pairs `(S₁, S₂)` of subsets of `Fin n`.  `S₁` records
positions cut at the child root; `S₂` records partial-trunk cuts that
keep the root and prune the inner leaf; the complement records fully
kept positions. -/
private def threeChoice (n : ℕ) :
    Finset (Finset (Fin n) × Finset (Fin n)) :=
  (Finset.univ : Finset (Finset (Fin n) × Finset (Fin n))).filter
    (fun p => Disjoint p.1 p.2)

/-- Trunk forest for a disjoint pair `(S₁, S₂)`: walk `Fin n` in order,
drop `S₁` entries, emit `node []` for `S₂` entries and `node [leaf]`
otherwise. -/
private def trunkChildren (n : ℕ) (S₁ S₂ : Finset (Fin n)) : List BTree :=
  ((List.finRange n).filter (fun i => i ∉ S₁)).map fun i =>
    if i ∈ S₂ then BTree.node [] else BTree.node [BTree.leaf]

/-- Per-pair contribution to the parametric closed form. -/
private noncomputable def repSingletonLeafContrib
...
-- TODO cycle 593+: prove by induction on `n` using `List.replicate_succ`
-- and the per-position cut decomposition; see strategy in
-- `.prover-state/strategy.md` (cycle 592).
...
(fully cut at the child root, partial-trunk cut keeping the root,
fully kept).  The three-way per-position cut is encoded by a disjoint
pair `(S₁, S₂) ∈ threeChoice n`. -/
...
/-- Sanity bridge: the parametric RHS at `n = 1` agrees with the
`bSeriesConvAug` value on `BTree.node [BTree.node [BTree.leaf]]`. -/
theorem bSeriesConvAug_node_replicate_singleton_leaf_one
    (α β : AugSeries) :
    α.toFun (BTree.node (List.replicate 1 (BTree.node [BTree.leaf])))
        * β.emptyVal
      + ∑ p ∈ threeChoice 1,
          repSingletonLeafContrib α β 1 p.1 p.2
    = bSeriesConvAug α β (BTree.node [BTree.node [BTree.leaf]]) := by
  rw [bSeriesConvAug_singleton_singleton_leaf]
  have h : threeChoice 1 =
      ({((∅ : Finset (Fin 1)), (∅ : Finset (Fin 1))),
        ((∅ : Finset (Fin 1)), ({0} : Finset (Fin 1))),
        (({0} : Finset (Fin 1)), (∅ : Finset (Fin 1)))} :
        Finset (Finset (Fin 1) × Finset (Fin 1))) := by decide
  rw [h]
  simp [repSingletonLeafContrib, trunkChildren, List.replicate, List.finRange]
  ring
...
theorem bSeriesConvAug_node_replicate_singleton_leaf_two
  ...
  rw [bSeriesConvAug_two_singleton_leaves]
  have h : threeChoice 2 =
      ({((∅ : Finset (Fin 2)), (∅ : Finset (Fin 2))),
        ((∅ : Finset (Fin 2)), ({0} : Finset (Fin 2))),
        ((∅ : Finset (Fin 2)), ({1} : Finset (Fin 2))),
        ((∅ : Finset (Fin 2)), ({0, 1} : Finset (Fin 2))),
        (({0} : Finset (Fin 2)), (∅ : Finset (Fin 2))),
        (({0} : Finset (Fin 2)), ({1} : Finset (Fin 2))),
        (({1} : Finset (Fin 2)), (∅ : Finset (Fin 2))),
        (({1} : Finset (Fin 2)), ({0} : Finset (Fin 2))),
        (({0, 1} : Finset (Fin 2)), (∅ : Finset (Fin 2)))} :
        Finset (Finset (Fin 2) × Finset (Fin 2))) := by decide
  rw [h]
