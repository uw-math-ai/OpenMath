...
Quantitative scalar convergence chain for the implicit Adams--Moulton 2-step
method.  Unlike the explicit Adams--Bashforth chains, the implicit update is
parameterised by a supplied trajectory satisfying the AM2 recurrence; fixed
point existence for that trajectory is a separate frontier theorem and is
not part of this chain.

The chain mirrors the AB2/AB3 templates in `LMMAB2Convergence` /
`LMMAB3Convergence`: rewrite the local truncation operator in textbook form,
prove a one-step Lipschitz recurrence, divide out the implicit
`(1 − (5/12)·h·L)` factor, bound the local residual via the fourth-order
Taylor remainder helpers exposed from `LMMAB3Convergence`, and assemble the
global error via `lmm_error_bound_from_local_truncation`. -/
...
/-- Max-norm AM2 one-step recurrence on the rolling pair `(en, en1)`.  Under
the small-step regime `(5/12)·h·L ≤ 1/2`, the maximum of consecutive errors
satisfies `EN (n+1) ≤ (1 + h·(3L)) · EN n + 2·|τ_n|`. -/
theorem am2_one_step_error_pair_bound
    {h L : ℝ} (hh : 0 ≤ h) (hL : 0 ≤ L)
    (hsmall : (5 / 12 : ℝ) * h * L ≤ 1 / 2)
    {f : ℝ → ℝ → ℝ}
    (hf : ∀ t a b : ℝ, |f t a - f t b| ≤ L * |a - b|)
  ...
  -- en2 bound from `am2_one_step_error_bound`.
  have hen2_bd :
      en2 ≤ (1 + h * (3 * L)) * E + 2 * τabs := by
    simpa [hen_def, hen1_def, hen2_def, hτabs_def, hE_def] using
      (am2_one_step_error_bound (h := h) (L := L) hh hL hsmall hf t₀ hy y hyf n)
  -- en1 ≤ E ≤ (1 + h(3L)) * E ≤ (1 + h(3L)) * E + 2 * τabs.
  have hen1_bd : en1 ≤ (1 + h * (3 * L)) * E + 2 * τabs := by
    have hone_le : E ≤ (1 + h * (3 * L)) * E := by
      have : (1 : ℝ) * E ≤ (1 + h * (3 * L)) * E :=
        mul_le_mul_of_nonneg_right (by linarith) hE_nn
      linarith
    linarith [hen1_le_E]
  exact max_le hen1_bd hen2_bd

/-- Pointwise AM2 truncation residual bound.  Algebraic identity
`R = R_y(2) − R_y(1) − (5h/12)·R_y'(2) − (8h/12)·R_y'(1)`, then bound each
term by the fourth-order Taylor remainder.  The residual is at most
`(11/8)·M·h⁴`, and we slack to `2·M·h⁴`. -/
theorem am2_pointwise_residual_bound
  ...
  have hRy1 :=
    y_fourth_order_taylor_remainder hy hbnd ht hth hh
  ...
  -- Algebraic identity.
  have hLTE_eq :
      y2 - y1 - h * ((5 / 12) * d2 + (8 / 12) * d1 - (1 / 12) * d0)
        = (y2 - y0 - (2 * h) * d0
  ...
  set B := y1 - y0 - h * d0 - h ^ 2 / 2 * dd - h ^ 3 / 6 * ddd with hB_def
  set C := d2 - d0 - (2 * h) * dd - (2 * h) ^ 2 / 2 * ddd with hC_def
  set D := d1 - d0 - h * dd - h ^ 2 / 2 * ddd with hD_def
  have h5h_nn : 0 ≤ 5 * h / 12 := by linarith
  have h8h_nn : 0 ≤ 8 * h / 12 := by linarith
  have habs5 : |(5 * h / 12) * C| = (5 * h / 12) * |C| := by
    rw [abs_mul, abs_of_nonneg h5h_nn]
  have habs8 : |(8 * h / 12) * D| = (8 * h / 12) * |D| := by
    rw [abs_mul, abs_of_nonneg h8h_nn]
  have htri : |A - B - (5 * h / 12) * C - (8 * h / 12) * D|
      ≤ |A| + |B| + (5 * h / 12) * |C| + (8 * h / 12) * |D| := by
    have h1 : |A - B - (5 * h / 12) * C - (8 * h / 12) * D|
        ≤ |A - B - (5 * h / 12) * C| + |(8 * h / 12) * D| :=
      abs_sub _ _
    have h2 : |A - B - (5 * h / 12) * C|
        ≤ |A - B| + |(5 * h / 12) * C| := abs_sub _ _
    have h3 : |A - B| ≤ |A| + |B| := abs_sub _ _
    linarith [habs5.le, habs5.ge, habs8.le, habs8.ge]
  have hA_bd : |A| ≤ M / 24 * (2 * h) ^ 4 := hRy2
  have hB_bd : |B| ≤ M / 24 * h ^ 4 := hRy1
  have hC_bd : |C| ≤ M / 6 * (2 * h) ^ 3 := hRyp2
  have hD_bd : |D| ≤ M / 6 * h ^ 3 := hRyp1
  have h5C_bd : (5 * h / 12) * |C| ≤ (5 * h / 12) * (M / 6 * (2 * h) ^ 3) :=
    mul_le_mul_of_nonneg_left hC_bd h5h_nn
  have h8D_bd : (8 * h / 12) * |D| ≤ (8 * h / 12) * (M / 6 * h ^ 3) :=
    mul_le_mul_of_nonneg_left hD_bd h8h_nn
  -- Sum of upper bounds: 16/24 + 1/24 + 40/72 + 8/72
  --                  = 17/24 + 48/72 = 17/24 + 16/24 = 33/24 = 11/8.
  have hbound_alg :
      M / 24 * (2 * h) ^ 4 + M / 24 * h ^ 4
        + (5 * h / 12) * (M / 6 * (2 * h) ^ 3)
        + (8 * h / 12) * (M / 6 * h ^ 3)
...
/-- Uniform bound on the AM2 one-step truncation residual on a finite
...
theorem am2_local_residual_bound
  ...
  -- with `k ≤ 2` (using `(n+2)*h ≤ T` and `h ≤ 1`).
  obtain ⟨M, hM_nn, hM⟩ :=
  ...
  rw [am2_localTruncationError_eq]
  show |y (t + 2 * h) - y (t + h)
      - h * (5 / 12 * deriv y (t + 2 * h)
            + 8 / 12 * deriv y (t + h)
            - 1 / 12 * deriv y t)|
  ...
  have hreshape :
      h * (5 / 12 * deriv y (t + 2 * h)
        ...
        = h * ((5 / 12) * deriv y (t + 2 * h)
              + (8 / 12) * deriv y (t + h)
              - (1 / 12) * deriv y t) := by ring
  rw [hreshape]
  exact am2_pointwise_residual_bound hy hM ht_mem hth_mem ht2h_mem hh.le

/-- Headline AM2 global error bound.  Given a supplied AM2 trajectory and
starting errors bounded by `ε₀`, the scalar global error is `O(ε₀ + h^3)` on
a finite horizon. -/
theorem am2_global_error_bound
  ...
  obtain ⟨C, δ, hC_nn, hδ_pos, hresidual⟩ :=
    am2_local_residual_bound hy_smooth t₀ T hT
  ...
  -- EN (n+1) ≤ (1 + h*(3L)) * EN n + 2*C * h^4.
  have hstep_general : ∀ n : ℕ, ((n : ℝ) + 2) * h ≤ T →
      EN (n + 1) ≤ (1 + h * (3 * L)) * EN n + (2 * C) * h ^ 4 := by
    intro n hnh_le
    have honestep := am2_one_step_error_pair_bound
    ...
    have h2τ : 2 * |adamsMoulton2.localTruncationError h
        (t₀ + (n : ℝ) * h) y| ≤ (2 * C) * h ^ 4 := by
