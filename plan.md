...
- [~] `thm:515D` **Stability and Consistency Imply Convergence (515D)** (§515) — OpenMath/Chapter5/Section515.lean (cycle 120: closed Path A of `.prover-state/issues/aux_515D_iterated_V_bound.md` by introducing and proving `aux_515D_iterated_V_bound` at Section515.lean:1854 — a `private theorem` axiom-clean to standard Lean axioms. The new helper converts `M.IsStable`'s power-bound `‖V^k‖ ≤ C` into the sup'-form vector bound `sup'_i |((V^k) *ᵥ x) i| ≤ (r·C) · sup'_j |x j|` needed by `aux_515D_max_deviation_geometric_bound`'s body. ~70 LOC proof; uses entrywise Frobenius bound `|V^k_{i,j}| ≤ ‖V^k‖_F` (proved inline via `Matrix.frobenius_norm_def` + `Real.sq_sqrt` + `abs_le_of_sq_le_sq` + `Real.rpow_two`), then `Finset.sup'_le` over the row sum. Avoids `Matrix.linfty_opNorm_mulVec` (which doesn't typecheck against the Frobenius default norm scope used in §515). Aristotle Job 4 submitted then cancelled after manual closure. Aristotle Jobs 2 (cycle 117 vector) and 3 (cycle 118 narrowed scalar) still IN_PROGRESS at cycle 120 close — re-poll in cycle 121. The geometric_bound sorry remains at Section515.lean:1961 (cycle 119's narrowing) — cycle 120 chose NOT to attempt Priority 2 body composition (~120 LOC + `0 ≤ c` upstream propagation = high risk for §513/§514 cascade integrity per Backup B3 of cycle 120 strategy). Cycle 121 target: close `aux_515D_max_deviation_geometric_bound`'s body using all available helpers (M-matrix cycle 114, localStepError cycle 116, per-step recurrence cycle 113, iterated-V cycle 120). Composition recipe documented in cycle 120 update sections of `.prover-state/issues/aux_515D_iterated_V_bound.md` and `.prover-state/issues/aux_515D_output_tendsto_hypotheses.md`. Build OK; `#print axioms stable_consistent_isConvergent` still has `sorryAx` (traces solely to `aux_515D_max_deviation_geometric_bound`).)
