...
- [~] `thm:515D` **Stability and Consistency Imply Convergence (515D)** (§515) — OpenMath/Chapter5/Section515.lean (cycle 123: closed body of `aux_515D_per_step_K_bound` (Section515.lean:1953, ~155 LOC) by specialising `localStepError_bound` (Lemma 515B) + `aux_515D_construct_ell_U_phi_A` (cycle 114). Threaded new hypothesis `_hc_le_one : ∀ i, M.glmAbscissae v i ≤ 1` through the §515D internal chain (per_step_K_bound, geometric_bound, max_deviation_bound_tendsto, componentwise_deviation_tendsto, output_tendsto) and extended `stable_consistent_isConvergent`'s `hc_nn_witness` to also produce `c_j ≤ 1`. Required because `localStepError_bound`'s localised M_bound clauses use `[xn1, xn1 + h·c_j]`, which must stay inside `[x₀, x]`; faithfulness divergence appended to `.prover-state/issues/stable_consistent_isConvergent_hc_nn.md`. NO §513/§514 cascade regression. `#print axioms` returns `[propext, sorryAx, Classical.choice, Quot.sound]`; tautology scanner: 0 hits. Sorry count: 1 in §515D (geometric_bound body remaining at line 2271). Cycle 124 — body of `aux_515D_max_deviation_geometric_bound` (~120-150 LOC: closed-form expansion + iterated V bound + sum-form Grönwall + α=0 case split) closes the §515D narrowing chain.)
