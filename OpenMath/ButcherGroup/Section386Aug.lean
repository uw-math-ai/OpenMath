...
theorem order_le_one_iff (c : BTree) :
...
def AugSeries.shiftBy (β : AugSeries) (c : BTree) : AugSeries where
  emptyVal := β.toFun (BTree.node [c])
  toFun := fun
    | BTree.leaf => 0
    | BTree.node xs => β.toFun (BTree.node (c :: xs))

@[simp] theorem AugSeries.shiftBy_node (β : AugSeries) (c : BTree)
    (xs : List BTree) :
...
`bSeriesConvAug`.  Exposed for split §386 augmented associativity
submodules; the public textbook-facing theorems still state expanded
node associativity. -/
noncomputable def forestSum (coeff : BTree → ℝ) (δ : AugSeries)
    (children : List BTree) : ℝ :=
  ((BTree.innerCutForest children coeff).map (fun forest =>
    forest.foldr (fun e acc => e.2 * acc) (1 : ℝ)
      * δ.toFun (BTree.node (forest.filterMap (fun e => e.1))))).sum

theorem forestSum_cons_depth_one_compact
...
theorem forestSum_cons_node_singleton_depth_one_compact
    (α β : AugSeries) (d : BTree) (hd : d.order ≤ 1) (cs : List BTree) :
...
lemma bSeriesConvAug_node_cons_depth_one_expand_compact
    (β γ : AugSeries) (c : BTree) (hc : c.order ≤ 1) (xs : List BTree) :
    bSeriesConvAug β γ (BTree.node (c :: xs))
      = β.toFun (BTree.node (c :: xs)) * γ.emptyVal
...
lemma bSeriesConvAug_node_cons_node_singleton_depth_one_expand_compact
    (β γ : AugSeries) (d : BTree) (hd : d.order ≤ 1) (xs : List BTree) :
    bSeriesConvAug β γ (BTree.node (BTree.node [d] :: xs))
      = β.toFun (BTree.node (BTree.node [d] :: xs)) * γ.emptyVal
