...
/-- *Phase α'.5.2.2 (cycle 501) — `mk [vertex, vertex, vertex, cherry]`
(order-6 first non-symmetric k=4 quadruple) closed form for Φ_{η_q⁻¹}.*

At the order-6 quadruple tree `mk [vertex, vertex, vertex, cherry]`, the
quotient-level elementary weight of the §383 inverse class admits the
closed form

  `Φ_{η_q⁻¹}(mk [v,v,v,c])
    = v⁶ - 5·v⁴·c + 3·v²·c² + 6·v³·b' - 3·v·c·b' - 4·v²·bu + c·bu
      + v·bushy₄ + v³·m - 3·v²·vc + 3·v·vvc - vvvc`

where `v, c, b', bu, bushy₄, m, vc, vvc, vvvc` abbreviate `Φ_η` at
vertex, cherry, broom₃, bushy, bushy₄, mk[cherry], mk[vertex,cherry],
mk[vertex,vertex,cherry], and the self-kernel mk[vertex,vertex,vertex,cherry]
respectively. The Aᵢ⁴-monomial in the per-row factor expansion surfaces
the `bushy₄` kernel — a 9th kernel beyond the strategy's initial
8-kernel sketch (per memory `feedback_dws_cherry_factor_includes_v_aᵢ.md`:
the cherry-component factor includes the `-v·Aᵢ` term).

First non-symmetric `k = 4` data point (Phase α'.5.2.2 per cycle 500
scoping doc §6.3). Combines cycle 370's bushy three-vertex-children
template (for the `(-v + Aᵢ)³` portion) with cycle 372's `mkVertexCherry`
cherry-tail template (for the `(v² - c) - v·Aᵢ + Bᵢ` portion).

Proof: descend via `Quotient.inductionOn`, apply
`elementaryWeightQ_phi_inv_mk` to expand the LHS, then unfold
`derivativeWeightWithSrc` via the four-children cons-case (outer three
vertex factors reusing cycle 367's `derivativeWeightWithSrc_vertex = 1`,
tail `[cherry]` reusing cycle 367's `(inv_v + ∑_k A_{jk})` formula).
The inner sum `∑_j A_{ij}·(inv_v + ∑_k A_{jk})` (linear distribution,
cycle 372 pattern) is pre-distributed via `h_inner_expand` before
`ring` can match the 9-term per-summand decomposition. Final assembly
via `Finset.sum_add_distrib × 8`, `← Finset.mul_sum × 8`, and
back-substitution into the nine kernels (vertex, cherry, broom₃,
bushy, bushy₄, mk[cherry], mk[vertex,cherry], mk[vertex,vertex,cherry],
mk[vertex,vertex,vertex,cherry]). -/
theorem elementaryWeightQ_phi_inv_mkVertexVertexVertexCherry
    (η_q : Quotient PhiEquivalent.setoidSigma) :
    elementaryWeightQ_phi (η_q⁻¹)
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
      ...
      = (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 6
        ...
        + 3 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
        + 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        - 3 * elementaryWeightQ_phi η_q RootedTree.vertex
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q RootedTree.broom₃
  ...
  have h_bushy : M.elementaryWeight RootedTree.bushy
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_bushy i]
  -- Cycle 499 helpers — bushy₄ four-vertex-children structure (reused).
  have h_dw_bushy₄ : ∀ i : Fin s,
      M.derivativeWeight i RootedTree.bushy₄ = (∑ j : Fin s, M.A i j) ^ 4 := by
    intro i
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
  ...
  have h_mkVertexCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexCherry i]; ring
  -- Cycle 403 helpers (reused verbatim) — `mk [vertex, vertex, cherry]` three-children
  -- asymmetric structure.
  have h_dw_mkVertexVertexCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
  ...
  have h_mkVertexVertexCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexCherry i]; ring
  -- New cycle 501 helpers — four-children asymmetric `mk [vertex, vertex, vertex, cherry]`
  -- structure: three-vertex prefix giving `Aᵢ³` factor + cherry tail giving the
  -- `Bᵢ`-pattern factor.
  have h_dw_mkVertexVertexVertexCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
             RootedTree.cherry])
    ...
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
          * M.derivativeWeightProd i
              [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry] = _
    have h_vertex_inner :
        ∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex
          = ∑ j : Fin s, M.A i j := by
      refine Finset.sum_congr rfl (fun j _ => ?_)
    ...
    have h_prod_step_vvc :
      ...
      show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
            * M.derivativeWeightProd i [RootedTree.vertex, RootedTree.cherry] = _
      rw [h_vertex_inner, h_prod_step_vc]; ring
    rw [h_vertex_inner, h_prod_step_vvc]; ring
  have h_mkVertexVertexVertexCherry : M.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
           RootedTree.cherry])
      = ∑ i : Fin s, M.b i * (∑ j : Fin s, M.A i j) ^ 3
    ...
    show ∑ i : Fin s, M.b i
          ...
          = ∑ i : Fin s, M.b i * (∑ j : Fin s, M.A i j) ^ 3
              * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k)
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexVertexCherry i]; ring
  -- Four-layer cons-case unfold for `mk [vertex, vertex, vertex, cherry]`'s
  -- derivativeWeightWithSrc: outer three-cons unfold strips the three-vertex
  -- prefix, producing three identical vertex-source factors `inv_v + Aᵢ`
  -- (cycle 367/370/499 pattern); inner cherry tail unfolds via cycle 367's
  -- `h_dws_cherry` to `inv_c + ∑ⱼ Aᵢⱼ · (inv_v + ∑ₖ Aⱼₖ)`.
  have h_dws_mkVertexVertexVertexCherry : ∀ i : Fin s,
      M.derivativeWeightWithSrc M.inverse i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
             RootedTree.cherry])
  ...
  have h_sum :
      (∑ i : Fin s, M.b i
        ...
        = -(M.elementaryWeight RootedTree.vertex) ^ 6
          ...
          - 3 * (M.elementaryWeight RootedTree.vertex) ^ 2
              * (M.elementaryWeight RootedTree.cherry) ^ 2
          - 6 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * M.elementaryWeight RootedTree.broom₃
          + 3 * M.elementaryWeight RootedTree.vertex
              * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight RootedTree.broom₃
        ...
        (∑ i : Fin s, M.b i
          ...
          = ∑ i : Fin s,
              ((-(M.elementaryWeight RootedTree.vertex) ^ 5
                  + (M.elementaryWeight RootedTree.vertex) ^ 3
                      * M.elementaryWeight RootedTree.cherry) * M.b i
                + (4 * (M.elementaryWeight RootedTree.vertex) ^ 4
                    - 3 * (M.elementaryWeight RootedTree.vertex) ^ 2
                        * M.elementaryWeight RootedTree.cherry)
                    * (M.b i * ∑ j : Fin s, M.A i j)
                + (-6 * (M.elementaryWeight RootedTree.vertex) ^ 3
                ...
                + M.b i * (∑ j : Fin s, M.A i j) ^ 3
                    * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k)) := by
      refine Finset.sum_congr rfl (fun i _ => ?_)
      rw [h_dws_mkVertexVertexVertexCherry i, h_inv_cherry, h_inv_v]
      -- Pre-distribute the inner Σⱼ Aᵢⱼ · (-v + ∑ₖ Aⱼₖ) before `ring` can
      -- match the 9-term per-summand decomposition.
      have h_inner_expand :
          ∑ j : Fin s, M.A i j
    ...
    rw [h_subst, Finset.sum_add_distrib, Finset.sum_add_distrib,
        ...
        ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum,
        ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum,
        ← h_mkVertexVertexVertexCherry, ← h_mkVertexVertexCherry,
        ← h_mkVertexCherry, ← h_mkCherry, ← h_bushy₄, ← h_bushy,
        ← h_broom₃, ← h_cherry, ← h_vertex]
    ring
  rw [h_sum]; ring

/-- *Phase α'.5.2.2 (cycle 501) — `mk [vertex, vertex, vertex, cherry]` closed
form non-vacuity at `η_q = ⟦explicitEuler⟧`.* The expected value is
`Φ_{⟦explicitEuler⟧⁻¹}(mk [v,v,v,c]) = 1⁶ - 5·1⁴·0 + 3·1²·0² + 6·1³·0
- 3·1·0·0 - 4·1²·0 + 0·0 + 1·0 + 1³·0 - 3·1²·0 + 3·1·0 - 0 = 1`, since
for explicit Euler `Σ b = 1` (so `Φ_η(vertex) = 1`) and `A = 0` (so all
higher elementary weights vanish). Leading `+v⁶` survives, all other
terms vanish — consistent with cycles 492's `mkVertexCherryCherry`
even-order pattern (order 6 ⇒ leading `+v⁶`). -/
example :
    elementaryWeightQ_phi
      ((Quotient.mk PhiEquivalent.setoidSigma
          ⟨1, RKTableau.explicitEuler⟩))⁻¹
  ...
  rw [elementaryWeightQ_phi_inv_mkVertexVertexVertexCherry, elementaryWeightQ_phi_mk,
      elementaryWeightQ_phi_mk, elementaryWeightQ_phi_mk,
      elementaryWeightQ_phi_mk, elementaryWeightQ_phi_mk,
      elementaryWeightQ_phi_mk, elementaryWeightQ_phi_mk,
      elementaryWeightQ_phi_mk, elementaryWeightQ_phi_mk]
    ...
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry]) = 0
    simp [h_mkVertexVertexCherry_zero]
  have h_mkVertexVertexVertexCherry_zero :
      RKTableau.explicitEuler.derivativeWeight 0
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
           RootedTree.cherry]) = 0 := by
    show (∑ j : Fin 1, RKTableau.explicitEuler.A 0 j
              * RKTableau.explicitEuler.derivativeWeight j RootedTree.vertex)
            * RKTableau.explicitEuler.derivativeWeightProd 0
                [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry] = 0
    simp [RKTableau.explicitEuler]
  have h_mkVertexVertexVertexCherry :
      RKTableau.explicitEuler.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
           RootedTree.cherry]) = 0 := by
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
                 RootedTree.cherry]) = 0
    simp [h_mkVertexVertexVertexCherry_zero]
  rw [h_vertex, h_cherry, h_broom₃, h_bushy, h_bushy₄, h_mkCherry,
      h_mkVertexCherry, h_mkVertexVertexCherry, h_mkVertexVertexVertexCherry]
  ring

/-- *Phase α'.5.2.2 (cycle 501) — Priority 2 `mk [vertex, vertex, vertex, cherry]`
m=0 witness:* specialisation of Sub-lemma A `powRep_sum_eq_of_strict_subtree_agreement`
at `t = mk [vertex, vertex, vertex, cherry], m = 0`. Under agreement at the
nine kernels `vertex, cherry, broom₃, bushy, bushy₄, mk [cherry],
mk [vertex, cherry], mk [vertex, vertex, cherry],
mk [vertex, vertex, vertex, cherry]` (corresponding to the closed-form
factors of cycle 501's `_mkVertexVertexVertexCherry`), the `η_q^(-1)`
images at `mk [vertex, vertex, vertex, cherry]` coincide.

Proof: reduce `η_q ^ (-(((0 + 1 : ℕ) : ℤ)))` to `η_q⁻¹` via `zero_add`
+ `Nat.cast_one` + `zpow_neg_one`, apply cycle 501's
`elementaryWeightQ_phi_inv_mkVertexVertexVertexCherry` on both sides,
then substitute the nine agreement hypotheses. -/
theorem powRep_sum_eq_of_agreement_at_mkVertexVertexVertexCherry_zero
    (η_q η_q' : Quotient PhiEquivalent.setoidSigma)
    (h_vertex : elementaryWeightQ_phi η_q RootedTree.vertex
              = elementaryWeightQ_phi η_q' RootedTree.vertex)
    (h_cherry : elementaryWeightQ_phi η_q RootedTree.cherry
    ...
    (h_mkVertexVertexCherry : elementaryWeightQ_phi η_q
              ...
              = elementaryWeightQ_phi η_q'
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry]))
    (h_mkVertexVertexVertexCherry : elementaryWeightQ_phi η_q
                                      (OpenMath.Chapter3.Section310.RootedTree.mk
                                        [RootedTree.vertex, RootedTree.vertex,
                                         RootedTree.vertex, RootedTree.cherry])
              = elementaryWeightQ_phi η_q'
  ...
  have h_pow : ∀ ζ : Quotient PhiEquivalent.setoidSigma,
    ...
    intro ζ; rw [zero_add, Nat.cast_one]; exact zpow_neg_one _
  rw [h_pow η_q, h_pow η_q',
      elementaryWeightQ_phi_inv_mkVertexVertexVertexCherry,
      elementaryWeightQ_phi_inv_mkVertexVertexVertexCherry,
      h_vertex, h_cherry, h_broom₃, h_bushy, h_bushy₄, h_mkCherry,
      h_mkVertexCherry, h_mkVertexVertexCherry, h_mkVertexVertexVertexCherry]

/-- *Phase α'.5.2.2 (cycle 501) — `mk [vertex, vertex, vertex, cherry]` m=0
witness non-vacuity at `η_q = η_q' = ⟦explicitEuler⟧`.* Discharges the
nine agreement hypotheses by `rfl`. -/
example :
    elementaryWeightQ_phi
        ((Quotient.mk PhiEquivalent.setoidSigma
            ⟨1, RKTableau.explicitEuler⟩) ^ (-(((0 + 1 : ℕ) : ℤ))))
      ...
      = elementaryWeightQ_phi
          ...
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
             RootedTree.cherry]) :=
  powRep_sum_eq_of_agreement_at_mkVertexVertexVertexCherry_zero
    (Quotient.mk PhiEquivalent.setoidSigma ⟨1, RKTableau.explicitEuler⟩)
    (Quotient.mk PhiEquivalent.setoidSigma ⟨1, RKTableau.explicitEuler⟩)
    rfl rfl rfl rfl rfl rfl rfl rfl rfl

/-- *Phase α'.5.2.3 (cycle 502) — `Φ_{η⁻¹}(mk [vertex, vertex, cherry, cherry])`
closed form in 12 named kernels.*

The per-row factor at `(t₁=v, t₂=v, t₃=c, t₄=c)` with
`inv_v = -v, inv_c = v² - c` evaluates to
`(Aᵢ - v)² · ((v² - c) - v·Aᵢ + Bᵢ)²`, expanding as a 18-monomial
sum in `(Aᵢ^j, Bᵢ^k)` with `(j,k) ∈ {0..4} × {0..2}`. Each monomial
contributes a coefficient × named kernel (12 kernels: `v, c, b', bu,
bushy₄, m, vc, vvc, vvvc, cc, vcc, vvcc`).

Phase α'.5.2.3 P1 deliverable. -/
theorem elementaryWeightQ_phi_inv_mkVertexVertexCherryCherry
    (η_q : Quotient PhiEquivalent.setoidSigma) :
    elementaryWeightQ_phi (η_q⁻¹)
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex,
           RootedTree.cherry, RootedTree.cherry])
      = -(elementaryWeightQ_phi η_q RootedTree.vertex) ^ 7
        + 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 5
            * elementaryWeightQ_phi η_q RootedTree.cherry
        - 7 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
        + 2 * elementaryWeightQ_phi η_q RootedTree.vertex
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 3
        - 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        + 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        ...
        - (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * elementaryWeightQ_phi η_q RootedTree.bushy₄
        - 2 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        + 2 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * elementaryWeightQ_phi η_q RootedTree.cherry
  ...
  have h_broom₃ : M.elementaryWeight RootedTree.broom₃
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_broom₃ i]
  -- Cycle 370 helpers (reused) — bushy three-vertex-children structure.
  have h_dw_bushy : ∀ i : Fin s,
      M.derivativeWeight i RootedTree.bushy = (∑ j : Fin s, M.A i j) ^ 3 := by
    intro i
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
  ...
  have h_mkCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkCherry i]
  -- Cycle 372 helpers (reused) — `mk [vertex, cherry]`.
  have h_dw_mkVertexCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.cherry])
  ...
  have h_mkVertexCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexCherry i]; ring
  -- Cycle 384 helpers (reused) — `mk [cherry, cherry]`.
  have h_dw_mkCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.cherry, RootedTree.cherry])
  ...
  have h_mkCherryCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkCherryCherry i]
  -- Cycle 403 helpers (reused) — `mk [vertex, vertex, cherry]`.
  have h_dw_mkVertexVertexCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
  ...
  have h_mkVertexVertexCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexCherry i]; ring
  -- Cycle 492 helpers (reused) — `mk [vertex, cherry, cherry]`.
  have h_dw_mkVertexCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
  ...
  have h_mkVertexCherryCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexCherryCherry i]; ring
  -- Cycle 501 helpers (reused) — `mk [vertex, vertex, vertex, cherry]`.
  have h_dw_mkVertexVertexVertexCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
  ...
  have h_mkVertexVertexVertexCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexVertexCherry i]; ring
  -- NEW cycle 502 helpers — four-children doubly-asymmetric
  -- `mk [vertex, vertex, cherry, cherry]` structure: two-vertex prefix giving
  -- `Aᵢ²` factor + two cherry tails giving `Bᵢ²` factor.
  have h_dw_mkVertexVertexCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
             RootedTree.cherry])
    ...
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
          * M.derivativeWeightProd i
              [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry] = _
    have h_vertex_inner :
        ∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex
          = ∑ j : Fin s, M.A i j := by
      refine Finset.sum_congr rfl (fun j _ => ?_)
    ...
    have h_prod_step_cc :
      ...
      show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.cherry)
            * M.derivativeWeightProd i [RootedTree.cherry] = _
      rw [h_cherry_inner, h_prod_step_cherry]; ring
    have h_prod_step_vcc :
        M.derivativeWeightProd i [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry]
          = (∑ j : Fin s, M.A i j)
            * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 2 := by
      show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
            * M.derivativeWeightProd i [RootedTree.cherry, RootedTree.cherry] = _
      rw [h_vertex_inner, h_prod_step_cc]
    rw [h_vertex_inner, h_prod_step_vcc]; ring
  have h_mkVertexVertexCherryCherry : M.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
           RootedTree.cherry])
      = ∑ i : Fin s, M.b i * (∑ j : Fin s, M.A i j) ^ 2
    ...
    show ∑ i : Fin s, M.b i
          ...
          = ∑ i : Fin s, M.b i * (∑ j : Fin s, M.A i j) ^ 2
              * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 2
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexCherryCherry i]; ring
  -- Four-layer cons-case unfold for `mk [v, v, c, c]`'s derivativeWeightWithSrc:
  -- outer two-cons strips the two-vertex prefix into a `(inv_v + Aᵢ)²` factor;
  -- inner two-cons strips the two-cherry tail into a
  -- `(inv_c + Σⱼ Aᵢⱼ · (inv_v + Σₖ Aⱼₖ))²` factor (cycle 384 + 492 pattern).
  have h_dws_mkVertexVertexCherryCherry : ∀ i : Fin s,
      M.derivativeWeightWithSrc M.inverse i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
             RootedTree.cherry])
    ...
    have h_prod_step_vcc :
      ...
      rw [h_vertex_inner, h_prod_step_cc]
    rw [h_vertex_inner, h_prod_step_vcc]; ring
  -- Main computation: assemble closed form via _inv_mk + _mk × 11 + sum algebra.
  rw [elementaryWeightQ_phi_inv_mk M
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
           RootedTree.cherry]),
  ...
  have h_sum :
      (∑ i : Fin s, M.b i
          * M.derivativeWeightWithSrc M.inverse i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
                 RootedTree.cherry]))
        = (M.elementaryWeight RootedTree.vertex) ^ 7
          - 6 * (M.elementaryWeight RootedTree.vertex) ^ 5
              * M.elementaryWeight RootedTree.cherry
          + 7 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * (M.elementaryWeight RootedTree.cherry) ^ 2
          - 2 * M.elementaryWeight RootedTree.vertex
              * (M.elementaryWeight RootedTree.cherry) ^ 3
          + 6 * (M.elementaryWeight RootedTree.vertex) ^ 4
              * M.elementaryWeight RootedTree.broom₃
          - 6 * (M.elementaryWeight RootedTree.vertex) ^ 2
              * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight RootedTree.broom₃
          ...
          = ∑ i : Fin s,
              ((M.elementaryWeight RootedTree.vertex) ^ 2
                ...
                + M.b i * (∑ j : Fin s, M.A i j) ^ 2
                    * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 2) := by
      refine Finset.sum_congr rfl (fun i _ => ?_)
      rw [h_dws_mkVertexVertexCherryCherry i, h_inv_cherry, h_inv_v]
      -- Pre-distribute the inner Σⱼ Aᵢⱼ · (-v + ∑ₖ Aⱼₖ) before `ring`
      -- can match the 12-term per-summand decomposition.
      have h_inner_expand :
          ∑ j : Fin s, M.A i j
              * (-M.elementaryWeight RootedTree.vertex + ∑ k : Fin s, M.A j k)
            = -M.elementaryWeight RootedTree.vertex * ∑ j : Fin s, M.A i j
    ...
    rw [h_subst, Finset.sum_add_distrib, Finset.sum_add_distrib,
        ...
        ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum,
        ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum,
        ← h_mkVertexVertexCherryCherry, ← h_mkVertexCherryCherry,
        ← h_mkVertexVertexVertexCherry, ← h_mkCherryCherry,
        ← h_mkVertexVertexCherry, ← h_mkVertexCherry, ← h_mkCherry,
        ← h_bushy₄, ← h_bushy, ← h_broom₃, ← h_cherry, ← h_vertex]
    ring
  rw [h_sum]; ring

/-- *Phase α'.5.2.3 (cycle 502) — `mk [v, v, c, c]` closed form non-vacuity at
`η_q = ⟦explicitEuler⟧`.* For explicit Euler, `Σ b = 1` and `A = 0`,
so all higher elementary weights vanish; order 7 (odd), leading `-v⁷`
survives. Value `-1`. -/
example :
    elementaryWeightQ_phi
      ((Quotient.mk PhiEquivalent.setoidSigma
    ...
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry]) = 0
    simp [h_mkVertexCherryCherry_zero]
  have h_mkVertexVertexCherryCherry_zero :
      RKTableau.explicitEuler.derivativeWeight 0
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
           RootedTree.cherry]) = 0 := by
    show (∑ j : Fin 1, RKTableau.explicitEuler.A 0 j
            ...
            * RKTableau.explicitEuler.derivativeWeightProd 0
                [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry] = 0
    simp [RKTableau.explicitEuler]
  have h_mkVertexVertexCherryCherry :
      RKTableau.explicitEuler.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
           RootedTree.cherry]) = 0 := by
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
                 RootedTree.cherry]) = 0
    simp [h_mkVertexVertexCherryCherry_zero]
  rw [h_vertex, h_cherry, h_broom₃, h_bushy, h_bushy₄, h_mkCherry,
      h_mkVertexCherry, h_mkVertexVertexCherry, h_mkVertexVertexVertexCherry,
      h_mkCherryCherry, h_mkVertexCherryCherry, h_mkVertexVertexCherryCherry]
  ring

/-- *Phase α'.5.2.3 (cycle 502) — `mk [v,v,c,c]` m=0 witness.* Specialisation
of Sub-lemma A `powRep_sum_eq_of_strict_subtree_agreement` at
`t = mk [v,v,c,c], m = 0`. Under agreement at the 12 kernels (the
closed-form factors of cycle 502's `_mkVertexVertexCherryCherry`),
the `η_q^(-1)` images at `mk [v,v,c,c]` coincide. Phase α'.5.2.3 P2
deliverable. -/
theorem powRep_sum_eq_of_agreement_at_mkVertexVertexCherryCherry_zero
    (η_q η_q' : Quotient PhiEquivalent.setoidSigma)
    (h_vertex : elementaryWeightQ_phi η_q RootedTree.vertex
              = elementaryWeightQ_phi η_q' RootedTree.vertex)
    (h_cherry : elementaryWeightQ_phi η_q RootedTree.cherry
    ...
    (h_mkVertexCherryCherry : elementaryWeightQ_phi η_q
              ...
              = elementaryWeightQ_phi η_q'
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry]))
    (h_mkVertexVertexCherryCherry : elementaryWeightQ_phi η_q
                                      (OpenMath.Chapter3.Section310.RootedTree.mk
                                        [RootedTree.vertex, RootedTree.vertex,
                                         RootedTree.cherry, RootedTree.cherry])
              = elementaryWeightQ_phi η_q'
  ...
  have h_pow : ∀ ζ : Quotient PhiEquivalent.setoidSigma,
    ...
    intro ζ; rw [zero_add, Nat.cast_one]; exact zpow_neg_one _
  rw [h_pow η_q, h_pow η_q',
      elementaryWeightQ_phi_inv_mkVertexVertexCherryCherry,
      elementaryWeightQ_phi_inv_mkVertexVertexCherryCherry,
      h_vertex, h_cherry, h_broom₃, h_bushy, h_bushy₄, h_mkCherry,
      h_mkVertexCherry, h_mkVertexVertexCherry, h_mkVertexVertexVertexCherry,
      h_mkCherryCherry, h_mkVertexCherryCherry, h_mkVertexVertexCherryCherry]

/-- *Phase α'.5.2.3 (cycle 502) — m=0 witness non-vacuity at
`η_q = η_q' = ⟦explicitEuler⟧`.* Discharges the 12 agreement
hypotheses by `rfl`. -/
example :
    elementaryWeightQ_phi
        ((Quotient.mk PhiEquivalent.setoidSigma
      ...
      = elementaryWeightQ_phi
          ...
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
             RootedTree.cherry]) :=
  powRep_sum_eq_of_agreement_at_mkVertexVertexCherryCherry_zero
    (Quotient.mk PhiEquivalent.setoidSigma ⟨1, RKTableau.explicitEuler⟩)
    (Quotient.mk PhiEquivalent.setoidSigma ⟨1, RKTableau.explicitEuler⟩)
    rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl

/-- *Phase α'.5.2.4 (cycle 503) — `Φ_{η⁻¹}(mk [vertex, cherry, cherry, cherry])`
closed form in 14 named kernels.*

The per-row factor at `(t₁=v, t₂=c, t₃=c, t₄=c)` with
`inv_v = -v, inv_c = v² - c` evaluates to
`(Aᵢ - v) · ((v² - c) - v·Aᵢ + Bᵢ)³`, expanding as a 14-term sum
in `(Aᵢ^j, Bᵢ^k)` with `(j,k) ∈ {0..4} × {0..3}`. Each non-zero
monomial contributes a coefficient × named kernel (14 kernels:
`v, c, b', bu, bushy₄, m, vc, vvc, vvvc, cc, vcc, vvcc, ccc, vccc`).

Two new kernels surface at this order:
* `ccc = mk [cherry, cherry, cherry]` (order 7, no prior infrastructure;
  the symmetric three-cherry-children analogue of cycle 384's
  `mk [cherry, cherry]`),
* `vccc = mk [vertex, cherry, cherry, cherry]` (order 8, the self-kernel
  of this very deliverable).

Phase α'.5.2.4 P1 deliverable. -/
theorem elementaryWeightQ_phi_inv_mkVertexCherryCherryCherry
    (η_q : Quotient PhiEquivalent.setoidSigma) :
    elementaryWeightQ_phi (η_q⁻¹)
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.cherry,
           RootedTree.cherry, RootedTree.cherry])
      = (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 8
        - 7 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 6
            * elementaryWeightQ_phi η_q RootedTree.cherry
        + 12 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
        - 7 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 3
        + (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 4
        + 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 5
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        - 9 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        + 3 * elementaryWeightQ_phi η_q RootedTree.vertex
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        - 4 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * elementaryWeightQ_phi η_q RootedTree.bushy
        + 3 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q RootedTree.bushy
        + (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * elementaryWeightQ_phi η_q RootedTree.bushy₄
        + 3 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 5
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        - 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * elementaryWeightQ_phi η_q RootedTree.cherry
        ...
        - 9 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry])
        + 12 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry])
        - 3 * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry])
        - 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
        + 3 * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
        - elementaryWeightQ_phi η_q
        ...
        + 9 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
        - 6 * elementaryWeightQ_phi η_q RootedTree.vertex
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
  ...
  have h_broom₃ : M.elementaryWeight RootedTree.broom₃
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_broom₃ i]
  -- Cycle 370 helpers — bushy three-vertex-children structure.
  have h_dw_bushy : ∀ i : Fin s,
      M.derivativeWeight i RootedTree.bushy = (∑ j : Fin s, M.A i j) ^ 3 := by
    intro i
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
  ...
  have h_bushy : M.elementaryWeight RootedTree.bushy
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_bushy i]
  -- Cycle 499 helpers — bushy₄ four-vertex-children structure.
  have h_dw_bushy₄ : ∀ i : Fin s,
      M.derivativeWeight i RootedTree.bushy₄ = (∑ j : Fin s, M.A i j) ^ 4 := by
    intro i
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
  ...
  have h_mkCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkCherry i]
  -- Cycle 372 helpers — `mk [vertex, cherry]`.
  have h_dw_mkVertexCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.cherry])
  ...
  have h_mkVertexCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexCherry i]; ring
  -- Cycle 384 helpers — `mk [cherry, cherry]`.
  have h_dw_mkCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.cherry, RootedTree.cherry])
  ...
  have h_mkCherryCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkCherryCherry i]
  -- Cycle 403 helpers — `mk [vertex, vertex, cherry]`.
  have h_dw_mkVertexVertexCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
  ...
  have h_mkVertexVertexCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexCherry i]; ring
  -- Cycle 492 helpers — `mk [vertex, cherry, cherry]`.
  have h_dw_mkVertexCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
  ...
  have h_mkVertexCherryCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexCherryCherry i]; ring
  -- Cycle 501 helpers — `mk [vertex, vertex, vertex, cherry]`.
  have h_dw_mkVertexVertexVertexCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
  ...
  have h_mkVertexVertexVertexCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexVertexCherry i]; ring
  -- Cycle 502 helpers — `mk [vertex, vertex, cherry, cherry]`.
  have h_dw_mkVertexVertexCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
  ...
  have h_mkVertexVertexCherryCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexCherryCherry i]; ring
  -- NEW cycle 503 helpers — three-cherry-children `mk [c, c, c]` (order 7,
  -- the symmetric three-cherry analogue of cycle 384's `mk [c, c]`).
  have h_dw_mkCherryCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
        = (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 3 := by
    intro i
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.cherry)
          * M.derivativeWeightProd i [RootedTree.cherry, RootedTree.cherry] = _
    have h_cherry_inner :
    ...
    have h_prod_step_cc :
      ...
      show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.cherry)
            * M.derivativeWeightProd i [RootedTree.cherry] = _
      rw [h_cherry_inner, h_prod_step_cherry]; ring
    rw [h_cherry_inner, h_prod_step_cc]; ring
  have h_mkCherryCherryCherry : M.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
      = ∑ i : Fin s, M.b i
          * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 3 := by
    show ∑ i : Fin s, M.b i
            * M.derivativeWeight i
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
          = ∑ i : Fin s, M.b i
              * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 3
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkCherryCherryCherry i]
  -- NEW cycle 503 helpers — four-children single-vertex-prefix
  -- `mk [vertex, cherry, cherry, cherry]` structure: one-vertex prefix giving
  -- `Aᵢ` factor + three cherry tails giving `Bᵢ³` factor.
  have h_dw_mkVertexCherryCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
             RootedTree.cherry])
    ...
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex)
          * M.derivativeWeightProd i
              [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry] = _
    have h_vertex_inner :
        ∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.vertex
          = ∑ j : Fin s, M.A i j := by
      refine Finset.sum_congr rfl (fun j _ => ?_)
    ...
    have h_prod_step_ccc :
      ...
      show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.cherry)
            * M.derivativeWeightProd i [RootedTree.cherry, RootedTree.cherry] = _
      rw [h_cherry_inner, h_prod_step_cc]; ring
    rw [h_vertex_inner, h_prod_step_ccc]
  have h_mkVertexCherryCherryCherry : M.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry])
      = ∑ i : Fin s, M.b i * (∑ j : Fin s, M.A i j)
    ...
    show ∑ i : Fin s, M.b i
          ...
          = ∑ i : Fin s, M.b i * (∑ j : Fin s, M.A i j)
              * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 3
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexCherryCherryCherry i]; ring
  -- Four-layer cons-case unfold for `mk [v, c, c, c]`'s derivativeWeightWithSrc:
  -- outer one-cons strips the one-vertex prefix into a `(inv_v + Aᵢ)` factor;
  -- inner three-cons strips the three-cherry tail into a
  -- `(inv_c + Σⱼ Aᵢⱼ · (inv_v + Σₖ Aⱼₖ))³` factor (cycle 384 + 492 pattern,
  -- cubed instead of squared).
  have h_dws_mkVertexCherryCherryCherry : ∀ i : Fin s,
      M.derivativeWeightWithSrc M.inverse i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
             RootedTree.cherry])
    ...
    have h_prod_step_ccc :
      ...
      rw [h_cherry_inner, h_prod_step_cc]; ring
    rw [h_vertex_inner, h_prod_step_ccc]
  -- Main computation: assemble closed form via _inv_mk + _mk × 14 + sum algebra.
  rw [elementaryWeightQ_phi_inv_mk M
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry]),
  ...
  have h_sum :
      (∑ i : Fin s, M.b i
          * M.derivativeWeightWithSrc M.inverse i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
                 RootedTree.cherry]))
        = -(M.elementaryWeight RootedTree.vertex) ^ 8
          + 7 * (M.elementaryWeight RootedTree.vertex) ^ 6
              * M.elementaryWeight RootedTree.cherry
          - 12 * (M.elementaryWeight RootedTree.vertex) ^ 4
              * (M.elementaryWeight RootedTree.cherry) ^ 2
          + 7 * (M.elementaryWeight RootedTree.vertex) ^ 2
              * (M.elementaryWeight RootedTree.cherry) ^ 3
          - (M.elementaryWeight RootedTree.cherry) ^ 4
          - 6 * (M.elementaryWeight RootedTree.vertex) ^ 5
              * M.elementaryWeight RootedTree.broom₃
          + 9 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight RootedTree.broom₃
          - 3 * M.elementaryWeight RootedTree.vertex
              * (M.elementaryWeight RootedTree.cherry) ^ 2
          ...
          - (M.elementaryWeight RootedTree.vertex) ^ 3
              * M.elementaryWeight RootedTree.bushy₄
          - 3 * (M.elementaryWeight RootedTree.vertex) ^ 5
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
          + 6 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * M.elementaryWeight RootedTree.cherry
          ...
          - M.elementaryWeight RootedTree.vertex
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
          + 9 * (M.elementaryWeight RootedTree.vertex) ^ 4
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry])
          - 12 * (M.elementaryWeight RootedTree.vertex) ^ 2
              * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry])
          + 3 * (M.elementaryWeight RootedTree.cherry) ^ 2
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry])
          + 6 * (M.elementaryWeight RootedTree.vertex) ^ 2
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
          - 3 * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
          + M.elementaryWeight
          ...
          - 9 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
          + 6 * M.elementaryWeight RootedTree.vertex
              * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
    ...
    have h_subst :
        (∑ i : Fin s, M.b i
          ...
          = ∑ i : Fin s,
              (-M.elementaryWeight RootedTree.vertex
                    * ((M.elementaryWeight RootedTree.vertex) ^ 2
                       - M.elementaryWeight RootedTree.cherry) ^ 3 * M.b i
                + ((((M.elementaryWeight RootedTree.vertex) ^ 2
                    - M.elementaryWeight RootedTree.cherry) ^ 3)
                    + 3 * (M.elementaryWeight RootedTree.vertex) ^ 2
                        * ((M.elementaryWeight RootedTree.vertex) ^ 2
                ...
                + (-3 * M.elementaryWeight RootedTree.vertex)
                    * (M.b i * (∑ j : Fin s, M.A i j) ^ 2
                        * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 2)
                + (-M.elementaryWeight RootedTree.vertex)
                    * (M.b i * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 3)
                + M.b i * (∑ j : Fin s, M.A i j)
                    * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 3) := by
      refine Finset.sum_congr rfl (fun i _ => ?_)
      rw [h_dws_mkVertexCherryCherryCherry i, h_inv_cherry, h_inv_v]
      -- Pre-distribute the inner Σⱼ Aᵢⱼ · (-v + ∑ₖ Aⱼₖ) before `ring`
      -- can match the 14-term per-summand decomposition.
      have h_inner_expand :
          ∑ j : Fin s, M.A i j
              * (-M.elementaryWeight RootedTree.vertex + ∑ k : Fin s, M.A j k)
            = -M.elementaryWeight RootedTree.vertex * ∑ j : Fin s, M.A i j
    ...
    rw [h_subst, Finset.sum_add_distrib, Finset.sum_add_distrib,
        ...
        ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum,
        ← Finset.mul_sum,
        ← h_mkVertexCherryCherryCherry, ← h_mkCherryCherryCherry,
        ← h_mkVertexVertexCherryCherry, ← h_mkVertexCherryCherry,
        ← h_mkVertexVertexVertexCherry, ← h_mkCherryCherry,
        ← h_mkVertexVertexCherry, ← h_mkVertexCherry, ← h_mkCherry,
        ← h_bushy₄, ← h_bushy, ← h_broom₃, ← h_cherry, ← h_vertex]
...
/-- *Phase α'.5.2.4 (cycle 503) — `mk [v, c, c, c]` closed form non-vacuity at
`η_q = ⟦explicitEuler⟧`.* For explicit Euler, `Σ b = 1` and `A = 0`,
so all higher elementary weights vanish; order 8 (even), leading `v⁸`
survives. Value `+1`. -/
example :
    elementaryWeightQ_phi
      ((Quotient.mk PhiEquivalent.setoidSigma
    ...
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.cherry, RootedTree.cherry]) = 0
    simp [h_mkCherryCherry_zero]
  have h_mkCherryCherryCherry_zero :
      RKTableau.explicitEuler.derivativeWeight 0
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry]) = 0 := by
    show (∑ j : Fin 1, RKTableau.explicitEuler.A 0 j
              * RKTableau.explicitEuler.derivativeWeight j RootedTree.cherry)
            * RKTableau.explicitEuler.derivativeWeightProd 0
                [RootedTree.cherry, RootedTree.cherry] = 0
    simp [RKTableau.explicitEuler]
  have h_mkCherryCherryCherry :
      RKTableau.explicitEuler.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry]) = 0 := by
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry]) = 0
    simp [h_mkCherryCherryCherry_zero]
  have h_mkVertexCherryCherry_zero :
      RKTableau.explicitEuler.derivativeWeight 0
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry]) = 0 := by
    ...
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry]) = 0
    simp [h_mkVertexCherryCherry_zero]
  have h_mkVertexCherryCherryCherry_zero :
      RKTableau.explicitEuler.derivativeWeight 0
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry]) = 0 := by
    show (∑ j : Fin 1, RKTableau.explicitEuler.A 0 j
            ...
            * RKTableau.explicitEuler.derivativeWeightProd 0
                [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry] = 0
    simp [RKTableau.explicitEuler]
  have h_mkVertexCherryCherryCherry :
      RKTableau.explicitEuler.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry]) = 0 := by
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
                 RootedTree.cherry]) = 0
    simp [h_mkVertexCherryCherryCherry_zero]
  have h_mkVertexVertexCherryCherry_zero :
      RKTableau.explicitEuler.derivativeWeight 0
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
  ...
  rw [h_vertex, h_cherry, h_broom₃, h_bushy, h_bushy₄, h_mkCherry,
      h_mkVertexCherry, h_mkVertexVertexCherry, h_mkVertexVertexVertexCherry,
      h_mkCherryCherry, h_mkCherryCherryCherry, h_mkVertexCherryCherry,
      h_mkVertexCherryCherryCherry, h_mkVertexVertexCherryCherry]
  ring

/-- *Phase α'.5.2.4 (cycle 503) — `mk [v,c,c,c]` m=0 witness.* Specialisation
of Sub-lemma A `powRep_sum_eq_of_strict_subtree_agreement` at
`t = mk [v,c,c,c], m = 0`. Under agreement at the 14 kernels (the
closed-form factors of cycle 503's `_mkVertexCherryCherryCherry`),
the `η_q^(-1)` images at `mk [v,c,c,c]` coincide. Phase α'.5.2.4 P2
deliverable. -/
theorem powRep_sum_eq_of_agreement_at_mkVertexCherryCherryCherry_zero
    (η_q η_q' : Quotient PhiEquivalent.setoidSigma)
    (h_vertex : elementaryWeightQ_phi η_q RootedTree.vertex
              = elementaryWeightQ_phi η_q' RootedTree.vertex)
    (h_cherry : elementaryWeightQ_phi η_q RootedTree.cherry
    ...
    (h_mkCherryCherry : elementaryWeightQ_phi η_q
              ...
              = elementaryWeightQ_phi η_q'
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.cherry, RootedTree.cherry]))
    (h_mkCherryCherryCherry : elementaryWeightQ_phi η_q
                                (OpenMath.Chapter3.Section310.RootedTree.mk
                                  [RootedTree.cherry, RootedTree.cherry,
                                   RootedTree.cherry])
              = elementaryWeightQ_phi η_q'
    ...
    (h_mkVertexVertexCherryCherry : elementaryWeightQ_phi η_q
              ...
              = elementaryWeightQ_phi η_q'
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.vertex,
                     RootedTree.cherry, RootedTree.cherry]))
    (h_mkVertexCherryCherryCherry : elementaryWeightQ_phi η_q
                                      (OpenMath.Chapter3.Section310.RootedTree.mk
                                        [RootedTree.vertex, RootedTree.cherry,
                                         RootedTree.cherry, RootedTree.cherry])
              = elementaryWeightQ_phi η_q'
  ...
  have h_pow : ∀ ζ : Quotient PhiEquivalent.setoidSigma,
    ...
    intro ζ; rw [zero_add, Nat.cast_one]; exact zpow_neg_one _
  rw [h_pow η_q, h_pow η_q',
      elementaryWeightQ_phi_inv_mkVertexCherryCherryCherry,
      elementaryWeightQ_phi_inv_mkVertexCherryCherryCherry,
      h_vertex, h_cherry, h_broom₃, h_bushy, h_bushy₄, h_mkCherry,
      h_mkVertexCherry, h_mkVertexVertexCherry, h_mkVertexVertexVertexCherry,
      h_mkCherryCherry, h_mkCherryCherryCherry, h_mkVertexCherryCherry,
      h_mkVertexVertexCherryCherry, h_mkVertexCherryCherryCherry]

/-- *Phase α'.5.2.4 (cycle 503) — m=0 witness non-vacuity at
`η_q = η_q' = ⟦explicitEuler⟧`.* Discharges the 14 agreement
hypotheses by `rfl`. -/
example :
    elementaryWeightQ_phi
        ((Quotient.mk PhiEquivalent.setoidSigma
      ...
      = elementaryWeightQ_phi
          ...
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
             RootedTree.cherry]) :=
  powRep_sum_eq_of_agreement_at_mkVertexCherryCherryCherry_zero
    (Quotient.mk PhiEquivalent.setoidSigma ⟨1, RKTableau.explicitEuler⟩)
    (Quotient.mk PhiEquivalent.setoidSigma ⟨1, RKTableau.explicitEuler⟩)
    rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl rfl

/-- *Phase α'.5.2.5 (cycle 504) — `Φ_{η⁻¹}(mk [cherry, cherry, cherry, cherry])`
closed form in 15 named kernels.*

The per-row factor at `(t₁=c, t₂=c, t₃=c, t₄=c)` with
`inv_c = v² - c` evaluates to `((v² - c) - v·Aᵢ + Bᵢ)⁴`, expanding as a
15-term sum in `(Aᵢ^j, Bᵢ^k)` with `(j,k) ∈ {0..4} × {0..4}` and
`j + k ≤ 4`. Each non-zero monomial contributes a coefficient × named
kernel (15 kernels: `v, c, b', bu, bushy₄, m, vc, vvc, vvvc, cc, vcc,
vvcc, ccc, vccc, cccc`; the new entry is the self-kernel `cccc`).

The all-cherry symmetric quadruple completes the symmetric quadruple
ladder (cycles 499/501/502/503: `bushy₄`, `mk[v,v,v,c]`, `mk[v,v,c,c]`,
`mk[v,c,c,c]`).

One new kernel surfaces at this order:
* `cccc = mk [cherry, cherry, cherry, cherry]` (order 9, no prior
  infrastructure; the self-kernel of this very deliverable).

Phase α'.5.2.5 P1 deliverable. -/
theorem elementaryWeightQ_phi_inv_mkCherryCherryCherryCherry
    (η_q : Quotient PhiEquivalent.setoidSigma) :
    elementaryWeightQ_phi (η_q⁻¹)
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry, RootedTree.cherry])
      = -(elementaryWeightQ_phi η_q RootedTree.vertex) ^ 9
        + 8 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 7
            * elementaryWeightQ_phi η_q RootedTree.cherry
        - 18 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 5
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
        + 16 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 3
        - 5 * elementaryWeightQ_phi η_q RootedTree.vertex
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 4
        - 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 6
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        + 12 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        - 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
            * elementaryWeightQ_phi η_q RootedTree.broom₃
        + 4 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 5
            * elementaryWeightQ_phi η_q RootedTree.bushy
        - 4 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q RootedTree.bushy
        - (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * elementaryWeightQ_phi η_q RootedTree.bushy₄
        - 4 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 6
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        + 12 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * elementaryWeightQ_phi η_q RootedTree.cherry
        ...
        - 12 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            ...
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        + 4 * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 3
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        - 6 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 4
            * elementaryWeightQ_phi η_q
        ...
        + 12 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            ...
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.cherry, RootedTree.cherry])
        - 6 * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.cherry, RootedTree.cherry])
        - 4 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 2
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
        + 4 * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
        - elementaryWeightQ_phi η_q
            (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
               RootedTree.cherry])
        + 12 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 5
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry])
        - 24 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * elementaryWeightQ_phi η_q RootedTree.cherry
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry])
        + 12 * elementaryWeightQ_phi η_q RootedTree.vertex
            * (elementaryWeightQ_phi η_q RootedTree.cherry) ^ 2
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry])
        + 12 * (elementaryWeightQ_phi η_q RootedTree.vertex) ^ 3
            * elementaryWeightQ_phi η_q
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
        - 12 * elementaryWeightQ_phi η_q RootedTree.vertex
  ...
  have h_bushy₄ : M.elementaryWeight RootedTree.bushy₄
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_bushy₄ i]
  -- Cycle 369: h_inv_cherry from quotient theorem.
  have h_inv_cherry : M.inverse.elementaryWeight RootedTree.cherry
      = (M.elementaryWeight RootedTree.vertex) ^ 2
        - M.elementaryWeight RootedTree.cherry :=
    elementaryWeightQ_phi_inv_cherry
      (Quotient.mk PhiEquivalent.setoidSigma ⟨s, M⟩)
  -- Cycle 369 helpers for `mk [cherry]`.
  have h_dw_mkCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        = ∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k := by
  ...
  have h_mkVertexVertexCherryCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexVertexCherryCherry i]; ring
  -- Cycle 503 helpers — `mk [c, c, c]` and `mk [v, c, c, c]`.
  have h_dw_mkCherryCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
  ...
  have h_mkVertexCherryCherryCherry : M.elementaryWeight
    ...
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkVertexCherryCherryCherry i]; ring
  -- NEW cycle 504 helpers — four-cherry-children `mk [c, c, c, c]` (order 9,
  -- the symmetric all-cherry quadruple, self-kernel of this deliverable).
  have h_dw_mkCherryCherryCherryCherry : ∀ i : Fin s,
      M.derivativeWeight i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
             RootedTree.cherry])
        = (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 4 := by
    intro i
    show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.cherry)
          * M.derivativeWeightProd i
              [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry] = _
    ...
    have h_prod_step_ccc :
      ...
      show (∑ j : Fin s, M.A i j * M.derivativeWeight j RootedTree.cherry)
            * M.derivativeWeightProd i [RootedTree.cherry, RootedTree.cherry] = _
      rw [h_cherry_inner, h_prod_step_cc]; ring
    rw [h_cherry_inner, h_prod_step_ccc]; ring
  have h_mkCherryCherryCherryCherry : M.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry])
      = ∑ i : Fin s, M.b i
            ...
            * M.derivativeWeight i
                (OpenMath.Chapter3.Section310.RootedTree.mk
                  [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
                   RootedTree.cherry])
          = ∑ i : Fin s, M.b i
              * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 4
    refine Finset.sum_congr rfl (fun i _ => ?_)
    rw [h_dw_mkCherryCherryCherryCherry i]
  -- Four-layer cons-case unfold for `mk [c, c, c, c]`'s derivativeWeightWithSrc:
  -- four-cons strips the four-cherry tail into a
  -- `(inv_c + Σⱼ Aᵢⱼ · (inv_v + Σₖ Aⱼₖ))⁴` factor (cycle 503 pattern, fourth
  -- power instead of cubed with no leading vertex prefix).
  have h_dws_mkCherryCherryCherryCherry : ∀ i : Fin s,
      M.derivativeWeightWithSrc M.inverse i
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
             RootedTree.cherry])
    ...
    have h_prod_step_ccc :
      ...
      rw [h_cherry_inner, h_prod_step_cc]; ring
    rw [h_cherry_inner, h_prod_step_ccc]; ring
  -- Main computation: assemble closed form via _inv_mk + _mk × 15 + sum algebra.
  rw [elementaryWeightQ_phi_inv_mk M
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry]),
  ...
  have h_sum :
      (∑ i : Fin s, M.b i
          * M.derivativeWeightWithSrc M.inverse i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
                 RootedTree.cherry]))
        = (M.elementaryWeight RootedTree.vertex) ^ 9
          - 8 * (M.elementaryWeight RootedTree.vertex) ^ 7
              * M.elementaryWeight RootedTree.cherry
          + 18 * (M.elementaryWeight RootedTree.vertex) ^ 5
              * (M.elementaryWeight RootedTree.cherry) ^ 2
          - 16 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * (M.elementaryWeight RootedTree.cherry) ^ 3
          + 5 * M.elementaryWeight RootedTree.vertex
              * (M.elementaryWeight RootedTree.cherry) ^ 4
          + 6 * (M.elementaryWeight RootedTree.vertex) ^ 6
              * M.elementaryWeight RootedTree.broom₃
          - 12 * (M.elementaryWeight RootedTree.vertex) ^ 4
              * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight RootedTree.broom₃
          + 6 * (M.elementaryWeight RootedTree.vertex) ^ 2
              * (M.elementaryWeight RootedTree.cherry) ^ 2
              * M.elementaryWeight RootedTree.broom₃
          - 4 * (M.elementaryWeight RootedTree.vertex) ^ 5
              * M.elementaryWeight RootedTree.bushy
          + 4 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight RootedTree.bushy
          + (M.elementaryWeight RootedTree.vertex) ^ 4
              * M.elementaryWeight RootedTree.bushy₄
          + 4 * (M.elementaryWeight RootedTree.vertex) ^ 6
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
          - 12 * (M.elementaryWeight RootedTree.vertex) ^ 4
              * M.elementaryWeight RootedTree.cherry
          ...
          + 12 * (M.elementaryWeight RootedTree.vertex) ^ 2
              ...
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
          - 4 * (M.elementaryWeight RootedTree.cherry) ^ 3
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
          + 6 * (M.elementaryWeight RootedTree.vertex) ^ 4
              * M.elementaryWeight
          ...
          - 12 * (M.elementaryWeight RootedTree.vertex) ^ 2
              ...
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.cherry, RootedTree.cherry])
          + 6 * (M.elementaryWeight RootedTree.cherry) ^ 2
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.cherry, RootedTree.cherry])
          + 4 * (M.elementaryWeight RootedTree.vertex) ^ 2
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
          - 4 * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
          + M.elementaryWeight
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
                 RootedTree.cherry])
          - 12 * (M.elementaryWeight RootedTree.vertex) ^ 5
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry])
          + 24 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * M.elementaryWeight RootedTree.cherry
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry])
          - 12 * M.elementaryWeight RootedTree.vertex
              * (M.elementaryWeight RootedTree.cherry) ^ 2
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry])
          - 12 * (M.elementaryWeight RootedTree.vertex) ^ 3
              * M.elementaryWeight
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
          + 12 * M.elementaryWeight RootedTree.vertex
    ...
    have h_subst :
        (∑ i : Fin s, M.b i
          ...
          = ∑ i : Fin s,
              ((((M.elementaryWeight RootedTree.vertex) ^ 2
                  - M.elementaryWeight RootedTree.cherry) ^ 4) * M.b i
                + (4 * ((M.elementaryWeight RootedTree.vertex) ^ 2
                       - M.elementaryWeight RootedTree.cherry) ^ 3)
                    * (M.b i * ∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k)
                + (6 * ((M.elementaryWeight RootedTree.vertex) ^ 2
                ...
                + ((M.elementaryWeight RootedTree.vertex) ^ 4)
                    * (M.b i * (∑ j : Fin s, M.A i j) ^ 4)
                + M.b i * (∑ j : Fin s, M.A i j * ∑ k : Fin s, M.A j k) ^ 4) := by
      refine Finset.sum_congr rfl (fun i _ => ?_)
      rw [h_dws_mkCherryCherryCherryCherry i, h_inv_cherry, h_inv_v]
      -- Pre-distribute the inner Σⱼ Aᵢⱼ · (-v + ∑ₖ Aⱼₖ) before `ring`
      -- can match the 15-term per-summand decomposition.
      have h_inner_expand :
          ∑ j : Fin s, M.A i j
              * (-M.elementaryWeight RootedTree.vertex + ∑ k : Fin s, M.A j k)
            = -M.elementaryWeight RootedTree.vertex * ∑ j : Fin s, M.A i j
    ...
    rw [h_subst, Finset.sum_add_distrib, Finset.sum_add_distrib,
        ...
        ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum, ← Finset.mul_sum,
        ← Finset.mul_sum, ← Finset.mul_sum,
        ← h_mkCherryCherryCherryCherry, ← h_mkVertexCherryCherryCherry,
        ← h_mkCherryCherryCherry, ← h_mkVertexCherryCherry,
        ← h_mkVertexVertexCherryCherry, ← h_mkVertexVertexVertexCherry,
        ← h_mkCherryCherry, ← h_mkVertexCherry, ← h_mkVertexVertexCherry,
        ← h_mkCherry, ← h_bushy₄, ← h_bushy, ← h_broom₃, ← h_cherry, ← h_vertex]
    ring
  rw [h_sum]; ring

/-- *Phase α'.5.2.5 (cycle 504) — `mk [c, c, c, c]` closed form non-vacuity at
`η_q = ⟦explicitEuler⟧`.* For explicit Euler, `Σ b = 1` and `A = 0`,
so all higher elementary weights vanish; order 9 (odd), leading `-v⁹`
survives. Value `-1`. -/
example :
    elementaryWeightQ_phi
      ((Quotient.mk PhiEquivalent.setoidSigma
    ...
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry]) = 0
    simp [h_mkCherryCherryCherry_zero]
  have h_mkCherryCherryCherryCherry_zero :
      RKTableau.explicitEuler.derivativeWeight 0
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry]) = 0 := by
    show (∑ j : Fin 1, RKTableau.explicitEuler.A 0 j
            ...
            * RKTableau.explicitEuler.derivativeWeightProd 0
                [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry] = 0
    simp [RKTableau.explicitEuler]
  have h_mkCherryCherryCherryCherry :
      RKTableau.explicitEuler.elementaryWeight
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
           RootedTree.cherry]) = 0 := by
    show ∑ i : Fin 1, RKTableau.explicitEuler.b i
            * RKTableau.explicitEuler.derivativeWeight i
              (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
                 RootedTree.cherry]) = 0
    simp [h_mkCherryCherryCherryCherry_zero]
  have h_mkVertexCherryCherry_zero :
      RKTableau.explicitEuler.derivativeWeight 0
        (OpenMath.Chapter3.Section310.RootedTree.mk
          [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry]) = 0 := by
  ...
  rw [h_vertex, h_cherry, h_broom₃, h_bushy, h_bushy₄, h_mkCherry,
      h_mkVertexCherry, h_mkVertexVertexCherry, h_mkVertexVertexVertexCherry,
      h_mkCherryCherry, h_mkCherryCherryCherry, h_mkCherryCherryCherryCherry,
      h_mkVertexCherryCherry, h_mkVertexCherryCherryCherry,
      h_mkVertexVertexCherryCherry]
  ring

/-- *Phase α'.5.2.5 (cycle 504) — `mk [c,c,c,c]` m=0 witness.* Specialisation
of Sub-lemma A `powRep_sum_eq_of_strict_subtree_agreement` at
`t = mk [c,c,c,c], m = 0`. Under agreement at the 15 kernels (the
closed-form factors of cycle 504's `_mkCherryCherryCherryCherry`),
the `η_q^(-1)` images at `mk [c,c,c,c]` coincide. Phase α'.5.2.5 P2
deliverable. -/
theorem powRep_sum_eq_of_agreement_at_mkCherryCherryCherryCherry_zero
    (η_q η_q' : Quotient PhiEquivalent.setoidSigma)
    (h_vertex : elementaryWeightQ_phi η_q RootedTree.vertex
              = elementaryWeightQ_phi η_q' RootedTree.vertex)
    (h_cherry : elementaryWeightQ_phi η_q RootedTree.cherry
    ...
    (h_mkVertexCherryCherryCherry : elementaryWeightQ_phi η_q
              ...
              = elementaryWeightQ_phi η_q'
                  (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry,
                     RootedTree.cherry, RootedTree.cherry]))
    (h_mkCherryCherryCherryCherry : elementaryWeightQ_phi η_q
                                      (OpenMath.Chapter3.Section310.RootedTree.mk
                                        [RootedTree.cherry, RootedTree.cherry,
                                         RootedTree.cherry, RootedTree.cherry])
              = elementaryWeightQ_phi η_q'
  ...
  have h_pow : ∀ ζ : Quotient PhiEquivalent.setoidSigma,
    ...
    intro ζ; rw [zero_add, Nat.cast_one]; exact zpow_neg_one _
  rw [h_pow η_q, h_pow η_q',
      elementaryWeightQ_phi_inv_mkCherryCherryCherryCherry,
      elementaryWeightQ_phi_inv_mkCherryCherryCherryCherry,
      h_vertex, h_cherry, h_broom₃, h_bushy, h_bushy₄, h_mkCherry,
      h_mkVertexCherry, h_mkVertexVertexCherry, h_mkVertexVertexVertexCherry,
      h_mkCherryCherry, h_mkCherryCherryCherry, h_mkCherryCherryCherryCherry,
      h_mkVertexCherryCherry, h_mkVertexVertexCherryCherry,
      h_mkVertexCherryCherryCherry]

/-- *Phase α'.5.2.5 (cycle 504) — m=0 witness non-vacuity at
`η_q = η_q' = ⟦explicitEuler⟧`.* Discharges the 15 agreement
hypotheses by `rfl`. -/
example :
    elementaryWeightQ_phi
        ((Quotient.mk PhiEquivalent.setoidSigma
      ...
      = elementaryWeightQ_phi
          ...
          (OpenMath.Chapter3.Section310.RootedTree.mk
            [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
             RootedTree.cherry]) :=
  powRep_sum_eq_of_agreement_at_mkCherryCherryCherryCherry_zero
    (Quotient.mk PhiEquivalent.setoidSigma ⟨1, RKTableau.explicitEuler⟩)
    (Quotient.mk PhiEquivalent.setoidSigma ⟨1, RKTableau.explicitEuler⟩)
...
/-- *Phase α'.5.2.1 (cycle 500), extended cycle 501 — quadruple-children
cross-term per-quadruple dispatch.*
...
* `(vertex, vertex, vertex, vertex)` →
  ...
  cross-term residual.
* `(vertex, vertex, vertex, cherry)` →
  `−v⁴·c + 6v³·b' − 3v·c·b' − 4v²·bu + c·bu + v·bushy₄
    + 3v·vvc − 3v²·vc`.
  Back-computed from cycle 501's
  `elementaryWeightQ_phi_inv_mkVertexVertexVertexCherry` 12-monomial
  closed form (with 9 named kernels including `bushy₄`) after
  subtracting the `tetrachildPolynomial` backbone at
  `(inv_v, inv_v, inv_v, inv_c) = (-v, -v, -v, v²−c)`. The three
  Block (2)/(3)/(4) bilinear self-term contributions at the
  three-vertex prefix each evaluate to `−(v² · (v²−c) · f cherry)
  = −v⁴c + v²c²`; their sum `−3v⁴c + 3v²c²` cancels partially against
  the closed form's `−5v⁴c + 3v²c²`, and Block (5) contributes
  `+v³·m`. The residual is the cycle 501 cross-term value above.
  Phase α'.5.2.2 P3 deliverable.
* `(vertex, vertex, cherry, cherry)` →
  `2v⁵·c − 2v³·c² − 6v⁴·b' + 6v²·c·b' − c²·b' + 4v³·bu − 2v·c·bu
    − v²·bushy₄ + 6v³·vc − 4v·c·vc − 6v²·vvc + 2c·vvc + 2v·vvvc
    − v²·cc + 2v·vcc`.
  Back-computed from cycle 502's
  `elementaryWeightQ_phi_inv_mkVertexVertexCherryCherry` 20-monomial
  closed form (with 12 named kernels) after subtracting the
  `tetrachildPolynomial` backbone at `(inv_v, inv_v, inv_c, inv_c)
  = (-v, -v, v²−c, v²−c)`. The `mk[c]` (m) kernel cancels here:
  Blocks (4)+(5) contribute `−2v⁴·m + 2v²·c·m`, exactly matching the
  closed form's m terms. Phase α'.5.2.3 P3 deliverable.
* `(vertex, cherry, cherry, cherry)` →
  `−3v⁶·c + 6v⁴·c² − 3v²·c³ + 6v⁵·b' − 9v³·c·b' + 3v·c²·b'
    − 4v⁴·bu + 3v²·c·bu + v³·bushy₄ + 3v³·cc − 3v·c·cc + v·ccc
    − 9v⁴·vc + 12v²·c·vc − 3c²·vc − 6v²·vcc + 3c·vcc
    + 9v³·vvc − 6v·c·vvc + 3v·vvcc − 3v²·vvvc`.
  Back-computed from cycle 503's
  `elementaryWeightQ_phi_inv_mkVertexCherryCherryCherry` 27-monomial
  closed form (with 14 named kernels, including two new ones `ccc` and
  the self-kernel `vccc`) after subtracting the `tetrachildPolynomial`
  backbone at `(inv_v, inv_c, inv_c, inv_c) = (-v, v²−c, v²−c, v²−c)`.
  Both the `mk[c]` (m) kernel and the self-kernel `vccc` cancel here:
  Blocks (3)+(4)+(5) contribute `+3v(v²−c)²·m`, exactly matching the
  closed form's m coefficient, and Block (16)'s `-vccc` cancels the
  closed form's `-vccc` structurally. Phase α'.5.2.4 P3 deliverable.
* Other quadruples → `0` (default; refined in cycle 504+ Phase α'.5.2.k
  as more quadruple-child witnesses surface).

Mirror of cycle 399's `trichildCrossTerm` design for the quadruple-child
case. Phase α'.5.2.1 P1 + Phase α'.5.2.2 P3 + Phase α'.5.2.3 P3 +
Phase α'.5.2.4 P3 deliverables. -/
noncomputable def tetrachildCrossTerm
  ...
  if t₁ = RootedTree.vertex ∧ t₂ = RootedTree.vertex
    ...
    -6 * (f RootedTree.vertex) ^ 2 * f RootedTree.broom₃
      ...
      ∧ t₃ = RootedTree.vertex ∧ t₄ = RootedTree.cherry then
    -(f RootedTree.vertex) ^ 4 * f RootedTree.cherry
      + 6 * (f RootedTree.vertex) ^ 3 * f RootedTree.broom₃
      - 3 * f RootedTree.vertex * f RootedTree.cherry * f RootedTree.broom₃
      - 4 * (f RootedTree.vertex) ^ 2 * f RootedTree.bushy
      + f RootedTree.cherry * f RootedTree.bushy
      + f RootedTree.vertex * f RootedTree.bushy₄
      + 3 * f RootedTree.vertex *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
      - 3 * (f RootedTree.vertex) ^ 2 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
      ...
      ∧ t₃ = RootedTree.cherry ∧ t₄ = RootedTree.cherry then
    2 * (f RootedTree.vertex) ^ 5 * f RootedTree.cherry
      - 2 * (f RootedTree.vertex) ^ 3 * (f RootedTree.cherry) ^ 2
      - 6 * (f RootedTree.vertex) ^ 4 * f RootedTree.broom₃
      + 6 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry * f RootedTree.broom₃
      - (f RootedTree.cherry) ^ 2 * f RootedTree.broom₃
      + 4 * (f RootedTree.vertex) ^ 3 * f RootedTree.bushy
      - 2 * f RootedTree.vertex * f RootedTree.cherry * f RootedTree.bushy
      - (f RootedTree.vertex) ^ 2 * f RootedTree.bushy₄
      + 6 * (f RootedTree.vertex) ^ 3 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
      - 4 * f RootedTree.vertex * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
      - 6 * (f RootedTree.vertex) ^ 2 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
      + 2 * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
      ...
      ∧ t₃ = RootedTree.cherry ∧ t₄ = RootedTree.cherry then
    -3 * (f RootedTree.vertex) ^ 6 * f RootedTree.cherry
      + 6 * (f RootedTree.vertex) ^ 4 * (f RootedTree.cherry) ^ 2
      - 3 * (f RootedTree.vertex) ^ 2 * (f RootedTree.cherry) ^ 3
      + 6 * (f RootedTree.vertex) ^ 5 * f RootedTree.broom₃
      - 9 * (f RootedTree.vertex) ^ 3 * f RootedTree.cherry * f RootedTree.broom₃
      + 3 * f RootedTree.vertex * (f RootedTree.cherry) ^ 2 * f RootedTree.broom₃
      - 4 * (f RootedTree.vertex) ^ 4 * f RootedTree.bushy
      + 3 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry * f RootedTree.bushy
      + (f RootedTree.vertex) ^ 3 * f RootedTree.bushy₄
      + 3 * (f RootedTree.vertex) ^ 3 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry])
      - 3 * f RootedTree.vertex * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry])
      + f RootedTree.vertex *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
      - 9 * (f RootedTree.vertex) ^ 4 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
      + 12 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
      - 3 * (f RootedTree.cherry) ^ 2 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
      - 6 * (f RootedTree.vertex) ^ 2 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
      + 3 * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
      + 9 * (f RootedTree.vertex) ^ 3 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
      - 6 * f RootedTree.vertex * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
      + 3 * f RootedTree.vertex *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
      ...
      - 3 * (f RootedTree.vertex) ^ 2 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
               RootedTree.cherry])
  else if t₁ = RootedTree.cherry ∧ t₂ = RootedTree.cherry
      ∧ t₃ = RootedTree.cherry ∧ t₄ = RootedTree.cherry then
    -- Cycle 504 — (c, c, c, c) cross-term: back-computed from
    -- cycle 504's `_mkCherryCherryCherryCherry` closed form minus the
    -- `tetrachildPolynomial` backbone at
    -- `(inv_c, inv_c, inv_c, inv_c) = (v²−c, v²−c, v²−c, v²−c)`. Three
    -- kernels cancel: `v` (Block 1's `-v · (v²−c)^4` absorbs the closed
    -- form's pure V-coefficient), `m` (Blocks 2+3+4+5 sum to
    -- `4(v²−c)^3 · m`, exactly matching the closed form's m coefficient),
    -- and `cccc` (Block 16's `-cccc` cancels the self-kernel structurally).
    -- The 25-monomial residual in 12 kernels (`c, b', bu, bushy₄, cc, ccc,
    -- vc, vcc, vccc, vvc, vvcc, vvvc`) is the cross-term value.
    -- Phase α'.5.2.5 P3 deliverable.
    4 * (f RootedTree.vertex) ^ 7 * f RootedTree.cherry
      - 12 * (f RootedTree.vertex) ^ 5 * (f RootedTree.cherry) ^ 2
      + 12 * (f RootedTree.vertex) ^ 3 * (f RootedTree.cherry) ^ 3
      - 4 * f RootedTree.vertex * (f RootedTree.cherry) ^ 4
      - 6 * (f RootedTree.vertex) ^ 6 * f RootedTree.broom₃
      + 12 * (f RootedTree.vertex) ^ 4 * f RootedTree.cherry * f RootedTree.broom₃
      - 6 * (f RootedTree.vertex) ^ 2 * (f RootedTree.cherry) ^ 2
          * f RootedTree.broom₃
      + 4 * (f RootedTree.vertex) ^ 5 * f RootedTree.bushy
      - 4 * (f RootedTree.vertex) ^ 3 * f RootedTree.cherry * f RootedTree.bushy
      - (f RootedTree.vertex) ^ 4 * f RootedTree.bushy₄
      - 6 * (f RootedTree.vertex) ^ 4 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry])
      + 12 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry])
      - 6 * (f RootedTree.cherry) ^ 2 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry])
      - 4 * (f RootedTree.vertex) ^ 2 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
      + 4 * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry])
      + 12 * (f RootedTree.vertex) ^ 5 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
      - 24 * (f RootedTree.vertex) ^ 3 * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
      + 12 * f RootedTree.vertex * (f RootedTree.cherry) ^ 2 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
      + 12 * (f RootedTree.vertex) ^ 3 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
      - 12 * f RootedTree.vertex * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry])
      + 4 * f RootedTree.vertex *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
               RootedTree.cherry])
      - 12 * (f RootedTree.vertex) ^ 4 *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry])
      + 12 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry *
          f (OpenMath.Chapter3.Section310.RootedTree.mk
...
`inversePolyTree (mk [vertex, vertex, vertex, cherry]) f` matches cycle
501's `elementaryWeightQ_phi_inv_mkVertexVertexVertexCherry` 12-monomial
closed form (in 9 named kernels) verbatim (under `f = elementaryWeightQ_phi η_q`).
The proof unfolds the quadruple-children branch of `inversePolyTree`
(cycle 500), rewrites `inversePolyTree vertex f = -f vertex` via
`inversePolyTree_vertex` thrice (all three occurrences via cycle 400's
single-rw discovery) and `inversePolyTree cherry f = (f vertex)² - f cherry`
via `inversePolyTree_cherry`, expands `tetrachildPolynomial`, and
dispatches `tetrachildCrossTerm`: the first if-branch
`(vertex, vertex, vertex, vertex)` fails (since `cherry ≠ vertex` in
the fourth component), and the second if-branch
`(vertex, vertex, vertex, cherry)` fires via `rfl`, exposing the cycle
501 cross-term value. Closure via a `show`-bridge that canonicalises
`f (mk [vertex]) ↔ f cherry` (from the cycle 500 Block (2)+(3)+(4)
expansion at `t₁ = t₂ = t₃ = vertex`), followed by `ring`.
Phase α'.5.2.2 P4 deliverable. -/
theorem inversePolyTree_mkVertexVertexVertexCherry (f : RT → ℝ) :
    inversePolyTree
      (OpenMath.Chapter3.Section310.RootedTree.mk
        [RootedTree.vertex, RootedTree.vertex, RootedTree.vertex,
         RootedTree.cherry]) f
          ...
          = -(f RootedTree.vertex) ^ 4 * f RootedTree.cherry
            ...
            - 3 * (f RootedTree.vertex) ^ 2 *
                f (OpenMath.Chapter3.Section310.RootedTree.mk
                    [RootedTree.vertex, RootedTree.cherry]) by
        unfold tetrachildCrossTerm
        rw [if_neg (by decide), if_pos ⟨rfl, rfl, rfl, rfl⟩]]
  show -(f RootedTree.vertex * -f RootedTree.vertex *
            -f RootedTree.vertex * -f RootedTree.vertex
            * ((f RootedTree.vertex) ^ 2 - f RootedTree.cherry))
        - -f RootedTree.vertex * -f RootedTree.vertex
        ...
        - -f RootedTree.vertex * -f RootedTree.vertex
            * ((f RootedTree.vertex) ^ 2 - f RootedTree.cherry)
            * f RootedTree.cherry
        - -f RootedTree.vertex * -f RootedTree.vertex * -f RootedTree.vertex
            * f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        + (-(f RootedTree.vertex) ^ 4 * f RootedTree.cherry
            + 6 * (f RootedTree.vertex) ^ 3 * f RootedTree.broom₃
            - 3 * f RootedTree.vertex * f RootedTree.cherry * f RootedTree.broom₃
...
/-- *Phase α'.5.2.3 (cycle 502) — `mk [vertex, vertex, cherry, cherry]`
calibration witness.*

`inversePolyTree (mk [vertex, vertex, cherry, cherry]) f` matches cycle
502's `elementaryWeightQ_phi_inv_mkVertexVertexCherryCherry`
20-monomial closed form (in 12 named kernels) verbatim under
`f = elementaryWeightQ_phi η_q`. Phase α'.5.2.3 P4 deliverable. -/
theorem inversePolyTree_mkVertexVertexCherryCherry (f : RT → ℝ) :
    inversePolyTree
      (OpenMath.Chapter3.Section310.RootedTree.mk
        [RootedTree.vertex, RootedTree.vertex, RootedTree.cherry,
         RootedTree.cherry]) f
      = -(f RootedTree.vertex) ^ 7
        + 6 * (f RootedTree.vertex) ^ 5 * f RootedTree.cherry
        - 7 * (f RootedTree.vertex) ^ 3 * (f RootedTree.cherry) ^ 2
        + 2 * f RootedTree.vertex * (f RootedTree.cherry) ^ 3
        - 6 * (f RootedTree.vertex) ^ 4 * f RootedTree.broom₃
        + 6 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry * f RootedTree.broom₃
        - (f RootedTree.cherry) ^ 2 * f RootedTree.broom₃
        + 4 * (f RootedTree.vertex) ^ 3 * f RootedTree.bushy
        - 2 * f RootedTree.vertex * f RootedTree.cherry * f RootedTree.bushy
        - (f RootedTree.vertex) ^ 2 * f RootedTree.bushy₄
        - 2 * (f RootedTree.vertex) ^ 4 *
            f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        + 2 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry *
            f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        + 6 * (f RootedTree.vertex) ^ 3 *
            f (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.vertex, RootedTree.cherry])
  ...
  rw [show tetrachildCrossTerm RootedTree.vertex RootedTree.vertex
          ...
          = 2 * (f RootedTree.vertex) ^ 5 * f RootedTree.cherry
            - 2 * (f RootedTree.vertex) ^ 3 * (f RootedTree.cherry) ^ 2
            - 6 * (f RootedTree.vertex) ^ 4 * f RootedTree.broom₃
            + 6 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry
                * f RootedTree.broom₃
            - (f RootedTree.cherry) ^ 2 * f RootedTree.broom₃
            + 4 * (f RootedTree.vertex) ^ 3 * f RootedTree.bushy
            - 2 * f RootedTree.vertex * f RootedTree.cherry * f RootedTree.bushy
...
/-- *Phase α'.5.2.4 (cycle 503) — `mk [vertex, cherry, cherry, cherry]`
calibration witness.*

`inversePolyTree (mk [vertex, cherry, cherry, cherry]) f` matches
cycle 503's `elementaryWeightQ_phi_inv_mkVertexCherryCherryCherry`
27-monomial closed form (in 14 named kernels, including two new ones
`ccc` and the self-kernel `vccc`) verbatim under
`f = elementaryWeightQ_phi η_q`. Phase α'.5.2.4 P4 deliverable. -/
theorem inversePolyTree_mkVertexCherryCherryCherry (f : RT → ℝ) :
    inversePolyTree
      (OpenMath.Chapter3.Section310.RootedTree.mk
        [RootedTree.vertex, RootedTree.cherry, RootedTree.cherry,
         RootedTree.cherry]) f
      = (f RootedTree.vertex) ^ 8
        - 7 * (f RootedTree.vertex) ^ 6 * f RootedTree.cherry
        + 12 * (f RootedTree.vertex) ^ 4 * (f RootedTree.cherry) ^ 2
        - 7 * (f RootedTree.vertex) ^ 2 * (f RootedTree.cherry) ^ 3
        + (f RootedTree.cherry) ^ 4
        + 6 * (f RootedTree.vertex) ^ 5 * f RootedTree.broom₃
        - 9 * (f RootedTree.vertex) ^ 3 * f RootedTree.cherry * f RootedTree.broom₃
        + 3 * f RootedTree.vertex * (f RootedTree.cherry) ^ 2 * f RootedTree.broom₃
        - 4 * (f RootedTree.vertex) ^ 4 * f RootedTree.bushy
        + 3 * (f RootedTree.vertex) ^ 2 * f RootedTree.cherry * f RootedTree.bushy
        + (f RootedTree.vertex) ^ 3 * f RootedTree.bushy₄
        + 3 * (f RootedTree.vertex) ^ 5 *
            f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        - 6 * (f RootedTree.vertex) ^ 3 * f RootedTree.cherry *
            f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
  ...
  rw [inversePolyTree, inversePolyTree_vertex, inversePolyTree_cherry]
  unfold tetrachildPolynomial
  rw [show tetrachildCrossTerm RootedTree.vertex RootedTree.cherry
            RootedTree.cherry RootedTree.cherry f
          = -3 * (f RootedTree.vertex) ^ 6 * f RootedTree.cherry
            + 6 * (f RootedTree.vertex) ^ 4 * (f RootedTree.cherry) ^ 2
            - 3 * (f RootedTree.vertex) ^ 2 * (f RootedTree.cherry) ^ 3
            + 6 * (f RootedTree.vertex) ^ 5 * f RootedTree.broom₃
            - 9 * (f RootedTree.vertex) ^ 3 * f RootedTree.cherry
                * f RootedTree.broom₃
            + 3 * f RootedTree.vertex * (f RootedTree.cherry) ^ 2
                * f RootedTree.broom₃
            - 4 * (f RootedTree.vertex) ^ 4 * f RootedTree.bushy
...
/-- *Phase α'.5.2.5 (cycle 504) — `mk [cherry, cherry, cherry, cherry]`
calibration witness.*

`inversePolyTree (mk [cherry, cherry, cherry, cherry]) f` matches
cycle 504's `elementaryWeightQ_phi_inv_mkCherryCherryCherryCherry`
31-monomial closed form (in 15 named kernels, with the new self-kernel
`cccc`) verbatim under `f = elementaryWeightQ_phi η_q`. Phase α'.5.2.5
P4 deliverable. -/
theorem inversePolyTree_mkCherryCherryCherryCherry (f : RT → ℝ) :
    inversePolyTree
      (OpenMath.Chapter3.Section310.RootedTree.mk
        [RootedTree.cherry, RootedTree.cherry, RootedTree.cherry,
         RootedTree.cherry]) f
      = -(f RootedTree.vertex) ^ 9
        + 8 * (f RootedTree.vertex) ^ 7 * f RootedTree.cherry
        - 18 * (f RootedTree.vertex) ^ 5 * (f RootedTree.cherry) ^ 2
        + 16 * (f RootedTree.vertex) ^ 3 * (f RootedTree.cherry) ^ 3
        - 5 * f RootedTree.vertex * (f RootedTree.cherry) ^ 4
        - 6 * (f RootedTree.vertex) ^ 6 * f RootedTree.broom₃
        + 12 * (f RootedTree.vertex) ^ 4 * f RootedTree.cherry * f RootedTree.broom₃
        - 6 * (f RootedTree.vertex) ^ 2 * (f RootedTree.cherry) ^ 2
            * f RootedTree.broom₃
        ...
        - 4 * (f RootedTree.vertex) ^ 3 * f RootedTree.cherry * f RootedTree.bushy
        - (f RootedTree.vertex) ^ 4 * f RootedTree.bushy₄
        - 4 * (f RootedTree.vertex) ^ 6 *
            f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        + 12 * (f RootedTree.vertex) ^ 4 * f RootedTree.cherry *
            f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        - 12 * (f RootedTree.vertex) ^ 2 * (f RootedTree.cherry) ^ 2 *
            f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        + 4 * (f RootedTree.cherry) ^ 3 *
            f (OpenMath.Chapter3.Section310.RootedTree.mk [RootedTree.cherry])
        - 6 * (f RootedTree.vertex) ^ 4 *
            f (OpenMath.Chapter3.Section310.RootedTree.mk
                [RootedTree.cherry, RootedTree.cherry])
  ...
  rw [inversePolyTree, inversePolyTree_cherry]
  unfold tetrachildPolynomial
  rw [show tetrachildCrossTerm RootedTree.cherry RootedTree.cherry
            RootedTree.cherry RootedTree.cherry f
          = 4 * (f RootedTree.vertex) ^ 7 * f RootedTree.cherry
            - 12 * (f RootedTree.vertex) ^ 5 * (f RootedTree.cherry) ^ 2
            + 12 * (f RootedTree.vertex) ^ 3 * (f RootedTree.cherry) ^ 3
        ...
        unfold tetrachildCrossTerm
        rw [if_neg (by decide), if_neg (by decide), if_neg (by decide),
            if_neg (by decide), if_pos ⟨rfl, rfl, rfl, rfl⟩]]
  show -(f RootedTree.vertex
            * ((f RootedTree.vertex) ^ 2 - f RootedTree.cherry)
            * ((f RootedTree.vertex) ^ 2 - f RootedTree.cherry)
            * ((f RootedTree.vertex) ^ 2 - f RootedTree.cherry)
            * ((f RootedTree.vertex) ^ 2 - f RootedTree.cherry))
...
/-- *Phase α'.5.2.1 (cycle 500), extended cycles 501 + 502 — `tetrachildCrossTerm`
respects closed-subtree agreement of its weight function.*

If `f g : RT → ℝ` agree on every subtree `s` with
`s.order ≤ (mk [c₁, c₂, c₃, c₄]).order`, then
`tetrachildCrossTerm c₁ c₂ c₃ c₄ f = tetrachildCrossTerm c₁ c₂ c₃ c₄ g`.

The non-default branches:

* `(vertex, vertex, vertex, vertex)` (cycle 500): references `f` at
  `vertex`, `broom₃`, and `bushy`, all of order ≤
  `(mk [vertex, vertex, vertex, vertex]).order = 5`.
* `(vertex, vertex, vertex, cherry)` (cycle 501): references `f` at
  `vertex`, `cherry`, `broom₃`, `bushy`, `bushy₄`,
  `mk [vertex, cherry]`, and `mk [vertex, vertex, cherry]`, all of
  order ≤ `(mk [vertex, vertex, vertex, cherry]).order = 6`.
* `(vertex, vertex, cherry, cherry)` (cycle 502): references `f` at
  `vertex`, `cherry`, `broom₃`, `bushy`, `bushy₄`,
  `mk [vertex, cherry]`, `mk [vertex, vertex, cherry]`,
  `mk [vertex, vertex, vertex, cherry]`, `mk [cherry, cherry]`, and
  `mk [vertex, cherry, cherry]`, all of order ≤
  `(mk [vertex, vertex, cherry, cherry]).order = 7`.

The default branch is `0 = 0`. Mandatory regression scope per cycle
500's "Discovery #2": any cycle adding a branch to `tetrachildCrossTerm`
MUST extend this lemma correspondingly, or
`inversePolyTree_eq_of_subtree_agreement` (cycle 497 Phase γ headline)
breaks. -/
private theorem tetrachildCrossTerm_eq_of_subtree_agreement
  ...
  · obtain ⟨h₁, h₂, h₃, h₄⟩ := h_vvvv
    ...
    rw [if_pos ⟨rfl, rfl, rfl, rfl⟩, if_pos ⟨rfl, rfl, rfl, rfl⟩,
        hv, hb, hbu]
  by_cases h_vvvc : c₁ = RootedTree.vertex ∧ c₂ = RootedTree.vertex
      ∧ c₃ = RootedTree.vertex ∧ c₄ = RootedTree.cherry
  · obtain ⟨h₁, h₂, h₃, h₄⟩ := h_vvvc
    subst h₁; subst h₂; subst h₃; subst h₄
    have hv : f RootedTree.vertex = g RootedTree.vertex :=
      h_closed RootedTree.vertex (by decide)
    have hc : f RootedTree.cherry = g RootedTree.cherry :=
    ...
    have hbu : f RootedTree.bushy = g RootedTree.bushy :=
      h_closed RootedTree.bushy (by decide)
    have hbu4 : f RootedTree.bushy₄ = g RootedTree.bushy₄ :=
      h_closed RootedTree.bushy₄ (by decide)
    have hvc :
        f (OpenMath.Chapter3.Section310.RootedTree.mk
              [RootedTree.vertex, RootedTree.cherry])
    ...
    rw [if_neg (by decide :
        ...
        if_neg (by decide :
            ¬ (RootedTree.vertex = RootedTree.vertex ∧
              ...
              RootedTree.cherry = RootedTree.vertex)),
        if_pos ⟨rfl, rfl, rfl, rfl⟩,
        hv, hc, hb, hbu, hbu4, hvc, hvvc]
  by_cases h_vvcc : c₁ = RootedTree.vertex ∧ c₂ = RootedTree.vertex
      ∧ c₃ = RootedTree.cherry ∧ c₄ = RootedTree.cherry
  · obtain ⟨h₁, h₂, h₃, h₄⟩ := h_vvcc
    subst h₁; subst h₂; subst h₃; subst h₄
    have hv : f RootedTree.vertex = g RootedTree.vertex :=
      h_closed RootedTree.vertex (by decide)
    have hc : f RootedTree.cherry = g RootedTree.cherry :=
    ...
    rw [if_neg (by decide :
        ...
        if_neg (by decide :
            ¬ (RootedTree.vertex = RootedTree.vertex ∧
              ...
              RootedTree.cherry = RootedTree.cherry)),
        if_pos ⟨rfl, rfl, rfl, rfl⟩,
        hv, hc, hb, hbu, hbu4, hvc, hvvc, hvvvc, hcc, hvcc]
  by_cases h_vccc : c₁ = RootedTree.vertex ∧ c₂ = RootedTree.cherry
      ∧ c₃ = RootedTree.cherry ∧ c₄ = RootedTree.cherry
  · obtain ⟨h₁, h₂, h₃, h₄⟩ := h_vccc
    subst h₁; subst h₂; subst h₃; subst h₄
    have hv : f RootedTree.vertex = g RootedTree.vertex :=
      h_closed RootedTree.vertex (by decide)
    have hc : f RootedTree.cherry = g RootedTree.cherry :=
    ...
    rw [if_neg (by decide :
        ...
        if_neg (by decide :
            ¬ (RootedTree.vertex = RootedTree.vertex ∧
              ...
              RootedTree.cherry = RootedTree.cherry)),
        if_pos ⟨rfl, rfl, rfl, rfl⟩,
        hv, hc, hb, hbu, hbu4, hvc, hvvc, hvvvc, hcc, hccc, hvcc, hvvcc]
  by_cases h_cccc : c₁ = RootedTree.cherry ∧ c₂ = RootedTree.cherry
      ∧ c₃ = RootedTree.cherry ∧ c₄ = RootedTree.cherry
  · obtain ⟨h₁, h₂, h₃, h₄⟩ := h_cccc
    subst h₁; subst h₂; subst h₃; subst h₄
    have hv : f RootedTree.vertex = g RootedTree.vertex :=
      h_closed RootedTree.vertex (by decide)
    have hc : f RootedTree.cherry = g RootedTree.cherry :=
    ...
    rw [if_neg (by decide :
        ...
        if_neg (by decide :
            ¬ (RootedTree.cherry = RootedTree.vertex ∧
              ...
              RootedTree.cherry = RootedTree.cherry)),
        if_pos ⟨rfl, rfl, rfl, rfl⟩,
        hv, hc, hb, hbu, hbu4, hvc, hvvc, hvvvc, hcc, hccc, hvcc, hvccc, hvvcc]
  · rw [if_neg h_vvvv, if_neg h_vvvc, if_neg h_vvcc, if_neg h_vccc, if_neg h_cccc,
        if_neg h_vvvv, if_neg h_vvvc, if_neg h_vvcc, if_neg h_vccc, if_neg h_cccc]
