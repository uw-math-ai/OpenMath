...
mixed leaf+node-nil, all-singleton-leaf,
...
`t₂`-permutation invariance lemmas) remains in `Section384.lean`. The mixed-root slices are in
`OpenMath.ButcherGroup.Section384SlicesMixed`. None of the lemmas moved
here is consumed by the core layer.
-/
