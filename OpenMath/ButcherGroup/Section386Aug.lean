...
/-- **Target 3 scaffold (cycle 587)**: unital associativity of
`bSeriesConvAug` at an arbitrary node.

This is intentionally left as the active §386 combinatorial scaffold: the
next proof step is to reindex the nested `(trunk, cuts)` decomposition of
`BTree.innerCutForest children` rather than the simpler two-branch
leaf-only powerset sum. -/
theorem mul_assoc_at_node
    (α β γ : AugSeries)
...
/-- **Target 1 API (cycle 587)**: the replicate-subtree instance of the
active `mul_assoc_at_node` scaffold. -/
theorem mul_assoc_at_node_replicate
    (α β γ : AugSeries)
    (hα : α.IsUnital) (hβ : β.IsUnital) (hγ : γ.IsUnital)
    (τ : BTree) (n : ℕ) :
    bSeriesConvAug ⟨1, fun σ => bSeriesConvAug α β σ⟩ γ
  ...
  simpa using mul_assoc_at_node α β γ hα hβ hγ (List.replicate n τ)

/-- Sanity check: the `n = 0` replicate-subtree instance is the already
closed empty-node theorem. -/
theorem mul_assoc_at_node_replicate_zero
    (α β γ : AugSeries)
    (hα : α.IsUnital) (hβ : β.IsUnital) (hγ : γ.IsUnital)
    (τ : BTree) :
    bSeriesConvAug ⟨1, fun σ => bSeriesConvAug α β σ⟩ γ
        (BTree.node (List.replicate 0 τ))
      = bSeriesConvAug α ⟨1, fun σ => bSeriesConvAug β γ σ⟩
          (BTree.node (List.replicate 0 τ)) := by
  simpa using mul_assoc_at_node_nil α β γ hα hβ hγ

/-- Sanity check: the `n = 1` replicate-subtree instance is the singleton
case of the active node scaffold. -/
theorem mul_assoc_at_node_replicate_one
    (α β γ : AugSeries)
    (hα : α.IsUnital) (hβ : β.IsUnital) (hγ : γ.IsUnital)
    (τ : BTree) :
    bSeriesConvAug ⟨1, fun σ => bSeriesConvAug α β σ⟩ γ
        (BTree.node (List.replicate 1 τ))
      = bSeriesConvAug α ⟨1, fun σ => bSeriesConvAug β γ σ⟩
          (BTree.node (List.replicate 1 τ)) := by
  simpa using mul_assoc_at_node_replicate α β γ hα hβ hγ τ 1

/-- **Target 2 (cycle 587)**: the two-identical-subtrees instance. -/
theorem mul_assoc_at_node_two_subtrees
    (α β γ : AugSeries)
    ...
    bSeriesConvAug ⟨1, fun σ => bSeriesConvAug α β σ⟩ γ
      ...
      = bSeriesConvAug α ⟨1, fun σ => bSeriesConvAug β γ σ⟩
          (BTree.node [τ, τ]) := by
  simpa using mul_assoc_at_node_replicate α β γ hα hβ hγ τ 2
