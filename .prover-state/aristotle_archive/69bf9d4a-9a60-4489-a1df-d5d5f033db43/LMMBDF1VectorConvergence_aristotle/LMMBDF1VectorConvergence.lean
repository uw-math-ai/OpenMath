...
private theorem y_second_order_taylor_remainder_vec
  ...
  have h_ftc : y (t + h) - y t - h • deriv y t = ∫ s in (0 : ℝ)..h, (deriv y (t + s) - deriv y t) := by
    rw [ intervalIntegral.integral_sub ] <;> norm_num;
    · rw [ intervalIntegral.integral_deriv_eq_sub ];
      · exact fun x hx => hy.contDiffAt.differentiableAt ( by norm_num );
      · exact ( hy.continuous_deriv ( by norm_num ) |> Continuous.intervalIntegrable ) _ _;
    · exact Continuous.intervalIntegrable ( by exact Continuous.comp ( hy.continuous_deriv ( by norm_num ) ) ( continuous_const.add continuous_id' ) ) _ _;
  -- By the properties of the integral, we can bound the norm of the integral.
  have h_integral_bound : ‖∫ s in (0 : ℝ)..h, (deriv y (t + s) - deriv y t)‖ ≤ ∫ s in (0 : ℝ)..h, M * s := by
    have h_integral_bound : ∀ s ∈ Set.Icc 0 h, ‖deriv y (t + s) - deriv y t‖ ≤ M * s := by
      intro s hs; by_cases hs' : s = 0 <;> simp_all +decide [ mul_comm ] ;
      have := derivY_first_order_taylor_remainder_vec hy ( fun u hu => hbnd u ( by linarith [ hu.1 ] ) ( by linarith [ hu.2 ] ) ) ( show t ∈ Set.Icc a b from ht ) ( show t + s ∈ Set.Icc a b from ⟨ by linarith, by linarith ⟩ ) hs.1; simp_all +decide [ mul_comm ] ;
  ...
  exact h_ftc ▸ h_integral_bound.trans_eq ( by norm_num [ mul_comm M ] ; ring )
