...
- [ ] `lem:441A` **Maximum order for a convergent k-step method** (§441) — cycle 173 added algebraic identities `aPoly_coeff_zero_of_preconsistent`, `aPoly_coeff_one_unconditional`, `aPoly_coeff_one_eq_neg_two_alpha_deriv_at_one_of_preconsistent`; cycle 174 added `ρPoly` definition + bridge lemmas `ρPoly_eval_one`, `ρPoly_eval_one_eq_zero_of_preconsistent`, `ρPoly_natDegree_le`, `ρPoly_deriv_eval_one_unconditional`, `ρPoly_deriv_eval_one_eq_neg_alpha_deriv_at_one_of_preconsistent`, the headline corollary `aPoly_coeff_one_eq_two_rho_deriv_at_one_of_preconsistent` (`a₁ = 2·ρ'(1)`), and non-vacuity `explicitEulerLMM_ρPoly_eq`; cycle 175 added Phase B.1.β `ρPoly_no_real_root_gt_one` (stability ⇒ no real root > 1, axiom-clean) plus BDF2 sanity witnesses `bdf2LMM_isPreconsistent`, `bdf2LMM_aPoly_coeff_zero_eq`, `bdf2LMM_aPoly_coeff_one_eq` (a₁ = 4/3); cycle 176 added Phase B.2 `ρPoly_deriv_eval_one_ne_zero_of_stable_preconsistent` (stability + preconsistency ⇒ `ρ'(1) ≠ 0`, axiom-clean) via private aux `idSeq_isHomogeneousSolution_of_preconsistent_ρPoly_deriv_zero` (the unbounded `n ↦ (n : ℝ)` solves (403a) under `ρ'(1) = 0` + preconsistency, contradicting `IsStable`'s boundedness) plus BDF2 sanity witness `bdf2LMM_ρPoly_deriv_eval_one_eq` (`ρ'(1) = 2/3`, consistent with cycle 175's `a₁ = 4/3 = 2·(2/3)` via cycle 174 bridge); the `a₁ > 0` half now reduces to `ρ'(1) > 0` for stable preconsistent LMMs, two cycles away (cycle 177 Phase B.3 ρ'(1) > 0 IVT-style sign assembly, cycle 178 Phase B.4 close a₁ > 0); BDF2 closed-form witness `bdf2LMM_aPoly_eq` permanently dropped — coefficient-level sanity now covered by `bdf2LMM_aPoly_coeff_one_eq` and `bdf2LMM_ρPoly_deriv_eval_one_eq`
