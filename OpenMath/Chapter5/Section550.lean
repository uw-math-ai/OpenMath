...
/-- **Theorem 550A at `n = 5`.** Expanding `det(I − z·X)` along the first row
twice (via `Matrix.det_succ_row_zero` for the 5×5 → 4×4 step and again for
each 4×4 → 3×3 step), then closing each 3×3 minor by `Matrix.det_fin_three`,
yields a polynomial of degree exactly `5` in `z`:
* `det(I - z X) = 1 + (α 0 + β 0) z + (α 0·β 0 + α 1 + β 1) z²
                    + (α 0·β 1 + α 1·β 0 + α 2 + β 2) z³
                    + (α 0·β 2 + α 1·β 1 + α 2·β 0 + α 3 + β 3) z⁴
...
* `α(z)·β(z) = (above)
                ...
                + (α 3·β 4 + α 4·β 3) z⁹ + (α 4·β 4) z¹⁰`,
so the residue is the convolution
`-(α 0·β 4 + α 1·β 3 + α 2·β 2 + α 3·β 1 + α 4·β 0) z⁶ - … - (α 4·β 4) z¹⁰
  = O(z^{5+1})`. This is the fifth concrete-`n` axiom-clean stepping stone
for Theorem 550A. -/
    ...
    Asymptotics.IsBigO (nhds (0 : ℂ))
      (fun z : ℂ =>
        (1 - z • doublyCompanionMatrix α β).det
          - alphaPoly α z * betaPoly β z)
      (fun z : ℂ => z ^ 6) := by
  -- Step 1: rewrite the residue pointwise to its factored form `z^6 * inner`.
  have h_diff : (fun z : ℂ =>
      (1 - z • doublyCompanionMatrix α β).det
        - alphaPoly α z * betaPoly β z)
      = (fun z : ℂ => z ^ 6 *
          (-(α 0 * β 4) - α 1 * β 3 - α 2 * β 2 - α 3 * β 1 - α 4 * β 0
            ...
            + z ^ 4 * (-(α 4 * β 4)))) := by
    funext z
    -- Reduce the doubly-companion matrix at n = 5 to an explicit !![…] form.
    have hX : doublyCompanionMatrix α β =
    ...
    -- Reduce `1 - z • X` to an explicit !![…] form.
    have hmat :
        (1 - z • !![-α 0, -α 1, -α 2, -α 3, -α 4 - β 4;
                    1,     0,    0,    0,    -β 3;
                    0,     1,    0,    0,    -β 2;
                    0,     0,    1,    0,    -β 1;
                    0,     0,    0,    1,    -β 0] : Matrix (Fin 5) (Fin 5) ℂ)
      ...
      fin_cases i <;> fin_cases j <;>
        first | (simp; ring) | simp
    rw [hmat]
    -- Expand the 5×5 determinant via Laplace along row 0, then expand each
    -- 4×4 minor by Laplace again, then use det_fin_three on the 3×3 minors.
    rw [Matrix.det_succ_row_zero]
    ...
    ring
  rw [h_diff]
  -- Step 2: prove `z^6 * inner =O[nhds 0] z^6`.
  refine Asymptotics.IsBigO.of_bound
      (‖-(α 0 * β 4) - α 1 * β 3 - α 2 * β 2 - α 3 * β 1 - α 4 * β 0‖
...
/-- **Theorem 550A at `n = 6`.** Expanding `det(I − z·X)` along the first row
three times (via `Matrix.det_succ_row_zero` for the 6×6 → 5×5 step, again for
the 5×5 → 4×4 step, again for the 4×4 → 3×3 step), then closing each 3×3 minor
by `Matrix.det_fin_three`, yields a polynomial of degree exactly `6` in `z`:
* `det(I - z X) = 1 + (α 0 + β 0) z + (α 0·β 0 + α 1 + β 1) z²
                    + (α 0·β 1 + α 1·β 0 + α 2 + β 2) z³
                    + (α 0·β 2 + α 1·β 1 + α 2·β 0 + α 3 + β 3) z⁴
                    + (α 0·β 3 + α 1·β 2 + α 2·β 1 + α 3·β 0 + α 4 + β 4) z⁵
...
* `α(z)·β(z) = (above)
                + (α 0·β 5 + α 1·β 4 + α 2·β 3 + α 3·β 2 + α 4·β 1
                   + α 5·β 0) z⁷
                + (α 1·β 5 + α 2·β 4 + α 3·β 3 + α 4·β 2 + α 5·β 1) z⁸
                + (α 2·β 5 + α 3·β 4 + α 4·β 3 + α 5·β 2) z⁹
                + (α 3·β 5 + α 4·β 4 + α 5·β 3) z¹⁰
                + (α 4·β 5 + α 5·β 4) z¹¹ + (α 5·β 5) z¹²`,
so the residue is the convolution
`-(α 0·β 5 + α 1·β 4 + α 2·β 3 + α 3·β 2 + α 4·β 1 + α 5·β 0) z⁷ - … - (α 5·β 5) z¹²
  = O(z^{6+1})`. This is the sixth concrete-`n` axiom-clean stepping stone
for Theorem 550A. -/
theorem doublyCompanionMatrix_det_factorization_n_six
    (α β : Fin 6 → ℂ) :
    Asymptotics.IsBigO (nhds (0 : ℂ))
      (fun z : ℂ =>
        (1 - z • doublyCompanionMatrix α β).det
          - alphaPoly α z * betaPoly β z)
      (fun z : ℂ => z ^ 7) := by
  -- Step 1: rewrite the residue pointwise to its factored form `z^7 * inner`.
  have h_diff : (fun z : ℂ =>
      (1 - z • doublyCompanionMatrix α β).det
        - alphaPoly α z * betaPoly β z)
      = (fun z : ℂ => z ^ 7 *
          (-(α 0 * β 5) - α 1 * β 4 - α 2 * β 3 - α 3 * β 2 - α 4 * β 1
              - α 5 * β 0
            + z * (-(α 1 * β 5) - α 2 * β 4 - α 3 * β 3 - α 4 * β 2 - α 5 * β 1)
            + z ^ 2 * (-(α 2 * β 5) - α 3 * β 4 - α 4 * β 3 - α 5 * β 2)
            + z ^ 3 * (-(α 3 * β 5) - α 4 * β 4 - α 5 * β 3)
            + z ^ 4 * (-(α 4 * β 5) - α 5 * β 4)
            + z ^ 5 * (-(α 5 * β 5)))) := by
    funext z
    -- Reduce the doubly-companion matrix at n = 6 to an explicit !![…] form.
    have hX : doublyCompanionMatrix α β =
    ...
    -- Reduce `1 - z • X` to an explicit !![…] form.
    have hmat :
        (1 - z • !![-α 0, -α 1, -α 2, -α 3, -α 4, -α 5 - β 5;
                    1,     0,    0,    0,    0,    -β 4;
                    0,     1,    0,    0,    0,    -β 3;
                    0,     0,    1,    0,    0,    -β 2;
                    0,     0,    0,    1,    0,    -β 1;
                    0,     0,    0,    0,    1,    -β 0] : Matrix (Fin 6) (Fin 6) ℂ)
      ...
      fin_cases i <;> fin_cases j <;>
        first | (simp; ring) | simp
    rw [hmat]
    -- Expand the 6×6 determinant via Laplace along row 0, then each 5×5
    -- minor by Laplace (n := 4), then each 4×4 minor by Laplace (n := 3),
    -- then close every 3×3 minor by det_fin_three.
    rw [Matrix.det_succ_row_zero]
    ...
    ring
  rw [h_diff]
  -- Step 2: prove `z^7 * inner =O[nhds 0] z^7`.
  refine Asymptotics.IsBigO.of_bound
      (‖-(α 0 * β 5) - α 1 * β 4 - α 2 * β 3 - α 3 * β 2 - α 4 * β 1 - α 5 * β 0‖
        + ‖-(α 1 * β 5) - α 2 * β 4 - α 3 * β 3 - α 4 * β 2 - α 5 * β 1‖
        + ‖-(α 2 * β 5) - α 3 * β 4 - α 4 * β 3 - α 5 * β 2‖
        + ‖-(α 3 * β 5) - α 4 * β 4 - α 5 * β 3‖
        + ‖-(α 4 * β 5) - α 5 * β 4‖
        + ‖-(α 5 * β 5)‖) ?_
  rw [Metric.eventually_nhds_iff]
  refine ⟨1, by norm_num, fun y hy => ?_⟩
  rw [Complex.dist_eq, sub_zero] at hy
  set a := -(α 0 * β 5) - α 1 * β 4 - α 2 * β 3 - α 3 * β 2 - α 4 * β 1
              - α 5 * β 0 with ha_def
  set b := -(α 1 * β 5) - α 2 * β 4 - α 3 * β 3 - α 4 * β 2 - α 5 * β 1
              with hb_def
  set c := -(α 2 * β 5) - α 3 * β 4 - α 4 * β 3 - α 5 * β 2 with hc_def
  set d := -(α 3 * β 5) - α 4 * β 4 - α 5 * β 3 with hd_def
  set e := -(α 4 * β 5) - α 5 * β 4 with he_def
  set f := -(α 5 * β 5) with hf_def
  -- Bound the inner factor:
  --   ‖a + y·b + y²·c + y³·d + y⁴·e + y⁵·f‖ ≤ ‖a‖ + ‖b‖ + ‖c‖ + ‖d‖ + ‖e‖ + ‖f‖.
  have h_inner :
    ...
    have hyf : ‖y ^ 5 * f‖ ≤ ‖f‖ := by
      ...
      calc ‖y‖ ^ 5 ≤ 1 ^ 5 := by gcongr
        _ = 1 := one_pow _
    have h1 : ‖a + y * b + y ^ 2 * c + y ^ 3 * d + y ^ 4 * e + y ^ 5 * f‖
                ≤ ‖a + y * b + y ^ 2 * c + y ^ 3 * d + y ^ 4 * e‖
                    + ‖y ^ 5 * f‖ := norm_add_le _ _
    have h2 : ‖a + y * b + y ^ 2 * c + y ^ 3 * d + y ^ 4 * e‖
                ≤ ‖a + y * b + y ^ 2 * c + y ^ 3 * d‖ + ‖y ^ 4 * e‖ :=
      norm_add_le _ _
    have h3 : ‖a + y * b + y ^ 2 * c + y ^ 3 * d‖
                ≤ ‖a + y * b + y ^ 2 * c‖ + ‖y ^ 3 * d‖ := norm_add_le _ _
    have h4 : ‖a + y * b + y ^ 2 * c‖
                ≤ ‖a + y * b‖ + ‖y ^ 2 * c‖ := norm_add_le _ _
    have h5 : ‖a + y * b‖ ≤ ‖a‖ + ‖y * b‖ := norm_add_le _ _
    linarith
  -- Multiply through by ‖y^7‖.
  rw [norm_mul]
  calc ‖y ^ 7‖ * ‖a + y * b + y ^ 2 * c + y ^ 3 * d + y ^ 4 * e + y ^ 5 * f‖
      ≤ ‖y ^ 7‖ * (‖a‖ + ‖b‖ + ‖c‖ + ‖d‖ + ‖e‖ + ‖f‖) :=
        mul_le_mul_of_nonneg_left h_inner (norm_nonneg _)
